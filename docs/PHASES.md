# Aivastra — Phased Development Plan
...
- `@aivastra/db` schema + drizzle-kit migrations (15 tables)
- `@aivastra/types` Zod schemas
- `@aivastra/storage` R2 presign provider
...
1. **End of Phase 0** — `@aivastra/types` Zod schemas locked. Both reference them; no changes after this point without a joint decision.
