...
**Repository:** `aivastra` pnpm monorepo
...
|---|---|
| `@aivastra/types` | `web`, `api`, `chatbot`, `dispatcher` |
| `@aivastra/db` | `api`, `chatbot`, `dispatcher` |
| `@aivastra/storage` | `api`, `dispatcher` |
| `@aivastra/logger` | `api`, `chatbot`, `dispatcher` |
| `@aivastra/observability` | `api`, `chatbot`, `dispatcher` |
...
```json
{
  ...
  changedPackages: [@aivastra/web],
  affectedPackages: [@aivastra/web],
  services: [web],
  testTargets: [@aivastra/web],
...
```text
ghcr.io/<owner>/aivastra-web
ghcr.io/<owner>/aivastra-admin
ghcr.io/<owner>/aivastra-shopify-admin
ghcr.io/<owner>/aivastra-api
ghcr.io/<owner>/aivastra-chatbot
ghcr.io/<owner>/aivastra-dispatcher
```
...
```text
ghcr.io/<owner>/aivastra-api@sha256:<digest>
```
...
```json
{
  ...
  images: {
    api: {
      reference: ghcr.io/<owner>/aivastra-api@sha256:<digest>,
      digest: sha256:<digest>
    },
    web: {
      reference: ghcr.io/<owner>/aivastra-web@sha256:<digest>,
      digest: sha256:<digest>
    }
  },
  migration: {
...
```text
/etc/aivastra/production.env                  root-owned runtime environment
/opt/aivastra/releases/<release-sha>/         immutable deployment bundles
/opt/aivastra/current                         symlink to latest successful bundle
/var/lib/aivastra-deploy/state.json           active slots and release state
/var/lib/aivastra-deploy/releases/            successful/failed release records
/var/lib/aivastra-deploy/gateway/             generated and previous gateway config
/var/lock/aivastra-deploy.lock                deployment lock
```
...
2. Copy it to `/opt/aivastra/releases/<sha>.incoming` using pinned SSH host keys.
3. Verify bundle checksum, manifest schema, release SHA, service allowlist, and image
   digests.
4. Atomically rename the directory to `/opt/aivastra/releases/<sha>`.
5. Acquire `/var/lock/aivastra-deploy.lock` with `flock`; never wait indefinitely.
