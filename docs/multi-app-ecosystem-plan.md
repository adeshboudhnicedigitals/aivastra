...
1. **`apps/virtual-tryon-mobile&kiosk`** — a native Android (Kotlin) kiosk-tablet app for in-store virtual try-on, currently wired to a completely different legacy backend (`https://api.aivastra.com/`, a CodeIgniter 3 PHP monolith, source snapshot at `legacy_ecosystem/`). It never talks to this monorepo's `apps/api`/`apps/dispatcher`/R2/ComfyUI stack at all.
...
|---|---|---|
| `app.aivastra.com` | `apps/catalogues-web` (customer web) — unchanged | `/v1/` → api:4000, `/minio/` → minio:9000 (existing) |
| `admin.aivastra.com` | `apps/admin-web` (Vite SPA) — moved from `app.aivastra.com/panel/` | `/admin/` and `/v1/` → api:4000 (Phase 1) |
| `merchant.aivastra.com` | `apps/merchant-web` (new Next.js app, extracted) | none — all API calls go through its own server-side BFF (Phase 2) |
...
|---|---|---|
| `admin.aivastra.com` | **Company staff only.** | Merchant accounts (approval, credits, kiosk enablement, linked-user bridge), workflow templates, GPU workers, and the **global shared data every catalogues-web user consumes** (model faces/backgrounds/poses, garment types, global lower/shoe catalog). Oversight/moderation of merchant content — never day-to-day merchant product management. |
| `merchant.aivastra.com` | Merchants. | Their widget (key, allowed origins, settings), their kiosk fleet (devices, pairing codes), their private kiosk catalog, their e-commerce integrations (Shopify/Wix — Phase 5), billing/credits, and management + kiosk-publishing of the catalogues they created in the studio (§7.3a). |
| `app.aivastra.com` | End users — including merchants acting as creators. | The studio wizard: uploading garments, generating try-on catalogues. This is where merchant catalogue **data is created**. |
...
## §6 Phase 1 — Subdomain Infra (admin.aivastra.com)
...
- New CloudPanel/NGINX vhost:
  ...
  admin.aivastra.com/admin/  → 127.0.0.1:4000   (api — dominant /admin/* prefix)
  admin.aivastra.com/v1/     → 127.0.0.1:4000   (api — covers the /v1/admin/widget-clients outlier)
  ```
...
- Optional: `return 301` on `app.aivastra.com/panel/*` → `https://admin.aivastra.com/$1` for old bookmarks.
- `.env.production`: `CORS_ORIGIN=https://app.aivastra.com,https://admin.aivastra.com`. Update `.env.production.example` to document the comma-separated format.
...
- `pnpm --filter @aivastra/admin build` (production) → built `index.html` references `/assets/...`, not `/panel/assets/...`.
...
## §7 Phase 2 — Merchant Portal Extraction + Unification (merchant.aivastra.com)

### 7.1 (2A) Extract the `(merchant)` route group into new `apps/merchant-web`

New Next.js 15 app, package `@aivastra/merchant`, `Dockerfile` mirrors `apps/catalogues-web/Dockerfile`.
...
> **Do NOT move `apps/catalogues-web/public/widget/loader.js`.** Verified: the embeddable widget script is served by `catalogues-web` at `app.aivastra.com/widget/loader.js`, and existing merchant e-commerce sites hotlink that exact URL. It is widget infrastructure, not merchant-portal UI — it stays in `catalogues-web` even though everything else merchant-flavored moves out. Moving or renaming it breaks every live embed on deploy. The dashboard's embed-snippet copy in `merchant-web` keeps generating the `app.aivastra.com` URL.
...
Deployment: new `merchant` service in `infra/docker-compose.prod.yml` (mirrors the `web` block, `127.0.0.1:3002:3000`). New vhost `merchant.aivastra.com/` → `127.0.0.1:3002` — **no `/v1/` proxy needed**: every merchant-portal→API call goes through the Next.js server-side BFF (confirmed: the `app/api/merchant/*/route.ts` files call Fastify server-side; the browser never hits Fastify directly for this surface). Add `https://merchant.aivastra.com` to `CORS_ORIGIN` **and** the MinIO bootstrap CORS JSON (needed for the portal's presigned catalog-image uploads, §7.3).
...
- Manual: `pnpm --filter @aivastra/merchant dev` — `/dashboard`, `/login`, `/api-keys` render and round-trip to a local `apps/api`.
- `pnpm --filter @aivastra/web build` still succeeds after `(merchant)` removal — no dangling imports in `catalogues-web`.
...
- The CORS/MinIO multi-origin change starts in Phase 1 and is **revisited in Phase 2** to add `merchant.aivastra.com` — both the API list *and* the MinIO bootstrap JSON, both times.

## §12 Deliberately Skipped (add only when the trigger fires)

- **Kiosk-vs-widget metrics label** in `@aivastra/observability` — `jobs.kioskDeviceId` makes the split queryable in SQL; add a Prometheus label only when a dashboard actually needs it.
