...
| Admin panel         | Next.js routes inside `apps/web`                      | Standalone Vite + React SPA (`apps/admin`) hitting `/admin/*`                                        |
...
The admin panel is a **standalone Vite + React SPA** (`apps/admin`), separate from `apps/web`. It
...
## 7. Frontend — User Flow (`apps/web`, Next.js 15)

Auth via httpOnly cookie (`access_token`); token refresh handled automatically in
`apps/web/src/lib/api.ts` (silent single-flight refresh, 1h idle timeout). App routes:
...
RESEND_API_KEY=...
EMAIL_FROM=noreply@aivastra.com
```
