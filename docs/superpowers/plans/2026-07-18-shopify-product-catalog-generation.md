...
Run: `pnpm --filter @aivastra/api test -- shopify-catalog-options`
...
Run: `pnpm --filter @aivastra/api exec tsc --noEmit -p .`
Expected: no errors.
...
Run: `pnpm --filter @aivastra/api test -- shopify-catalog-generate`
...
Run: `pnpm --filter @aivastra/api test -- shopify-catalog-jobs`
Expected: FAIL — 404.
...
**Interfaces:**
- Consumes: `decryptToken` (`../../lib/crypto.js`), `SHOPIFY_API_VERSION` (`./service.js`), `app.storage.presignGet(key, expiresIn)`, `keys.output(jobId)` (`@aivastra/storage`).
...
Run: `pnpm --filter @aivastra/api test -- shopify-catalog-publish`
...
Run: restart `pnpm --filter @aivastra/api dev` so it picks up the new `SHOPIFY_SCOPES` env value, then visit `/v1/shopify/auth?shop=<your-dev-store>.myshopify.com` in a browser to re-consent with the combined scope set.
...
Run: `pnpm --filter @aivastra/shopify-admin exec tsc --noEmit -p .`
Expected: no errors. (Verify the exact package name first — it was `@aivastra/shopify-admin` per prior work in this session, not `@aivastra/shopify`.)

- [ ] **Step 5: Manual smoke test**

Run: `pnpm --filter @aivastra/shopify-admin dev`, open the embedded app in a dev store via the Partner Dashboard, navigate to `/catalog-generate?productId=<a real synced product id>`, confirm: product images load, gender/garment-type/face/background/pose selections populate, "Generate" creates a catalogueId and starts polling, completed results show thumbnails with a working "Add to product" button.
    ...
    <AdminBlock title="AiVastra catalog images">
      ...
      {open && (
        <Modal
          ...
          src={`https://catalog-admin.aivastra.com/catalog-generate?productId=${productId}`}
...
The `Modal`'s `src` must point at wherever `apps/shopify` (Task 7) is actually served — replace `https://catalog-admin.aivastra.com` with the real `application_url` value from `shopify.app.dev.toml` for local dev, and the production `apps/shopify` deployment URL for `shopify.app.toml`. If `data.selected` is unavailable (unsaved "Add product" page — the open risk flagged in the design spec), `productId` will be empty and the button stays disabled; confirm this behavior against a real dev store in Step 4 and adjust the disabled-state messaging if Shopify's actual behavior differs.

- [ ] **Step 4: Run the extension locally and verify**

Run: `cd apps/shopify-extension && shopify app dev`
Expected: CLI prints a preview URL; opening a product's edit page in the connected dev store shows the "AiVastra catalog images" block with a "Generate catalog images" button; clicking it opens a modal iframing `CatalogGeneratePage` from Task 7.
