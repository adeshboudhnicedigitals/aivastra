...
- Run tests with `pnpm --filter @aivastra/api test`. Requires `pnpm docker:up` running first.
...
Run: `pnpm --filter @aivastra/api test -- graphql.test`
...
Run: `pnpm --filter @aivastra/api test -- collections`
...
Run: `pnpm --filter @aivastra/api typecheck && pnpm --filter @aivastra/api test -- shopify`
...
Run: `pnpm --filter @aivastra/api test -- shopify-products` then `pnpm --filter @aivastra/api test -- shopify-catalog-generate`
Expected: PASS both.
...
```toml
  ...
  uri = "https://app.aivastra.com/v1/shopify/webhooks/app_uninstalled"

  [[webhooks.subscriptions]]
  topics = [ "app_subscriptions/update" ]
  uri = "https://app.aivastra.com/v1/shopify/webhooks/app_subscriptions_update"

  [[webhooks.subscriptions]]
  topics = [ "products/update" ]
  uri = "https://app.aivastra.com/v1/shopify/webhooks/products_update"

  [[webhooks.subscriptions]]
  topics = [ "products/delete" ]
  uri = "https://app.aivastra.com/v1/shopify/webhooks/products_delete"
```
...
Run: `pnpm --filter @aivastra/api typecheck && pnpm --filter @aivastra/api test -- shopify-catalog`
...
- `pnpm --filter @aivastra/api test:unit` passes.
