...
**Interfaces:**
- Consumes: `schema.jobs`, `schema.shopifyStoreCredits`; `SIMPLE_TRYON_COST` from `@aivastra/types`.
- Produces:
...
Run: `pnpm --filter @aivastra/api test:unit -- runway`
...
pnpm docker:up
pnpm db:migrate
docker exec -i aivastra-postgres psql -U postgres -d aivastra -c "\d shopify_stores" | grep -E "shop_email|last_alert"
```
...
- [ ] `pnpm lint` — clean
- [ ] `pnpm build` — clean
- [ ] `pnpm --filter @aivastra/api test:unit` — pass
