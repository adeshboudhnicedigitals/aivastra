...
**Tech Stack:** Next.js 15 App Router, React 19, TypeScript 5.6. No test runner in `apps/catalogues-web` — verification is `pnpm --filter @aivastra/web typecheck`/`lint`/`build` plus manual browser checks, per this repo's established convention for frontend-only work.
...
Run: `pnpm --filter @aivastra/web lint`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/web dev`, open `/pricing` at 375px, 800px, and 1728px widths. Confirm the `Mobile`/`Tablet`/`Desktop` layouts still dispatch correctly exactly as before this refactor — this task must not change pricing's behavior at all, only `use-breakpoint.ts`'s internals.
...
Run: `pnpm --filter @aivastra/web dev`, open any `(app)` page (e.g. `/studio`) at ≥1024px. Confirm: permanent sidebar rail, identical to before this task — no hamburger visible anywhere (TopBar doesn't have one yet — that's Task 6, so there's nothing to check there yet), no drawer/backdrop markup present.
...
Run: `pnpm --filter @aivastra/web dev`.
...
Run: `pnpm --filter @aivastra/web typecheck` — expect PASS.
Run: `pnpm --filter @aivastra/web lint` — expect PASS.
Run: `pnpm --filter @aivastra/web build` — expect PASS.

- [ ] **Step 2: Manual sweep per the spec's testing plan**

With `pnpm --filter @aivastra/web dev` running:
