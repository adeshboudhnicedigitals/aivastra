...
**Tech Stack:** Fastify 5 + zod (`fastify-type-provider-zod`), Drizzle ORM / PostgreSQL, Redis (upload-ownership binding), R2/MinIO via `@aivastra/storage`, `sharp` for thumbnailing, Node 20 native `fetch`, Next.js 15 + React Query on the frontend, Vitest for tests.

## Global Constraints

- pnpm workspaces; ESM only (`"type": "module"`); Node 20+, TypeScript 5.6.
- No `console.log` — use the existing `app.log` (pino) in API code.
- No new lockfiles; only `pnpm add` if a dependency is genuinely missing (none needed here — `sharp` is already a dependency of `@aivastra/api`).
...
```bash
docker exec aivastra-postgres psql -U tryon -d tryon_dev -tAc "\d model_backgrounds" | grep user_id
```
...
```bash
pnpm --filter @aivastra/api test -- ssrf-guard
```
...
```bash
pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts -t "backgrounds/mine"
```
...
```bash
pnpm --filter @aivastra/types typecheck && pnpm --filter @aivastra/api typecheck
```
...
```bash
pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts -t "from-url"
```
...
```bash
pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts -t "user-scoped background ownership"
```
...
```bash
pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts -t "createJob"
```
...
```bash
pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts
```
