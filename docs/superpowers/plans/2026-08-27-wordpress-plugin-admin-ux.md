...
**Goal:** Bring the plugin's Settings → Aivastra Try-On admin page up to the
...
the three existing PHP classes (`Aivastra_Connection_Settings`,
`Aivastra_Connection_Service`, `Aivastra_Settings_Page`) and adds one plain
...
```
Run this after every task. If you ever add a NEW `.php` class file (this plan doesn't require one, but if you deviate), also run `docker run --rm -v "$(pwd):/app" -w /app composer:2 dump-autoload` first, and add a matching `require_once` line in `aivastra-tryon.php` — `PluginBootstrapTest::test_every_class_file_is_required_by_the_bootstrap` will fail loudly if you forget (this exact mistake happened earlier in this project and shipped a fatal error to a live site).

---

## Task 1: `Aivastra_Connection_Settings` — store and expose the credit balance
...
## Task 2: `Aivastra_Connection_Settings` — `update_snapshot()` and `clear()`
...
## Task 3: `Aivastra_Connection_Service` — capture credits on connect, add `refresh()`
...
## Task 4: `Aivastra_Settings_Page` — refresh and disconnect handlers
...
## Task 5: `Aivastra_Settings_Page::render()` rewrite + admin stylesheet
...
outside a container becomes a `.aivastra-card` div, and the `<h2>` moves
inside it):
...
```css
.aivastra-settings-wrap .aivastra-card {
  background: #fff;
...
.aivastra-inline-action > summary {
  list-style: none;
  cursor: pointer;
}

.aivastra-inline-action > summary::-webkit-details-marker {
  display: none;
}

.aivastra-inline-form {
  margin-top: 10px;
  padding: 14px;
  background: #f6f7f7;
  border: 1px solid #dcdcde;
...
.aivastra-btn-danger:hover {
  background: #b32d2e !important;
  color: #fff !important;
}

.aivastra-connect-step {
  display: flex;
  gap: 14px;
  margin-bottom: 20px;
}

.aivastra-step-number {
  flex-shrink: 0;
  width: 26px;
  height: 26px;
  border-radius: 50%;
...
.aivastra-step-body label {
  display: block;
  font-weight: 600;
  margin-bottom: 4px;
}

.aivastra-step-body .description {
  margin: 0 0 8px;
  color: #646970;
  font-size: 12.5px;
}

.aivastra-category-card h2 {
  margin-top: 0;
}
```
...
**Files:**
- Modify: `aivastra-tryon.php`
- Modify: `docs/progress.md`

- [ ] **Step 1: Bump the plugin version**

In `aivastra-tryon.php`, change both occurrences of the version string:
...
```bash
docker run --rm -v "$(pwd):/app" -w /app php:8.2-cli php -l aivastra-tryon.php
docker run --rm -v "$(pwd):/app" -w /app php:8.2-cli php vendor/bin/phpunit --testsuite unit
...
Reload `http://localhost:8888/wp-admin/options-general.php?page=aivastra-tryon`
...
```bash
git add wordpress-plugin/aivastra-tryon.php docs/progress.md
