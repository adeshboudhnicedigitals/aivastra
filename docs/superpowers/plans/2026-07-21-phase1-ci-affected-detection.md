...
- No `console.log` in committed application code. The detector is a CI script, not an application service — it writes to stdout deliberately and does not use `@aivastra/logger`.
...
`^2.1.3` matches `@aivastra/api` and `@aivastra/storage`. `packages/db` already pins `^4.1.7`, so mixed majors already coexist in this hoisted workspace; Step 6 verifies nothing regressed.
...
```bash
pnpm docker:up
pnpm --filter @aivastra/api test:unit
pnpm --filter @aivastra/dispatcher test:unit
```
...
describe('loadTargets', () => {
  ...
  it('maps directories that differ from their target name', () => {
    ...
    expect(byName.get('web')?.packageName).toBe('@aivastra/web');
    expect(byName.get('admin')?.dir).toBe('apps/admin-web');
    expect(byName.get('admin')?.packageName).toBe('@aivastra/admin');
    expect(byName.get('shopify-admin')?.dir).toBe('apps/shopify');
    expect(byName.get('shopify-admin')?.packageName).toBe('@aivastra/shopify-admin');
  });
...
```json
{
  ...
  "targets": [
    ...
    },
    {
      "name": "shopify-admin",
      "dir": "apps/shopify",
      "packageName": "@aivastra/shopify-admin",
    ...
    },
    {
      "name": "dispatcher",
      "dir": "apps/dispatcher",
      "packageName": "@aivastra/dispatcher",
  ...
  "testablePackages": ["@aivastra/api", "@aivastra/dispatcher"],
...
```json
{
  ...
  "targets": [
    ...
    },
    {
      "name": "api-clone",
      "dir": "apps/api",
      "packageName": "@aivastra/api-clone",
      "dockerfile": "apps/api/Dockerfile",
...
Computes recursive workspace consumers from `package.json` files so a `@aivastra/types` change selects exactly `web`, `api`, `chatbot`, `dispatcher` (§4.2) — derived from real edges, never a hardcoded table.
...
const FAKE_MANIFESTS: Record<string, string> = {
  'packages/types/package.json': JSON.stringify({ name: '@aivastra/types' }),
  'packages/db/package.json': JSON.stringify({
    name: '@aivastra/db',
    dependencies: { '@aivastra/types': 'workspace:*', drizzle: '^1.0.0' },
  }),
  'apps/api/package.json': JSON.stringify({
    name: '@aivastra/api',
    dependencies: { '@aivastra/db': 'workspace:*' },
    devDependencies: { vitest: '^2.1.3' },
  }),
  'apps/catalogues-web/package.json': JSON.stringify({
    name: '@aivastra/web',
    dependencies: { '@aivastra/types': 'workspace:*' },
  }),
...
describe('readWorkspaceMembers', () => {
  it('records the directory and workspace-protocol dependencies', () => {
    ...
    const api = members.find((m) => m.name === '@aivastra/api');
    expect(api?.dir).toBe('apps/api');
    expect(api?.workspaceDeps).toEqual(['@aivastra/db']);
  });
  ...
  it('ignores non-workspace dependencies', () => {
    const members = readWorkspaceMembers(Object.keys(FAKE_MANIFESTS), fakeRead);
    const db = members.find((m) => m.name === '@aivastra/db');
    expect(db?.workspaceDeps).toEqual(['@aivastra/types']);
  });
});
...
describe('recursiveConsumers', () => {
  ...
  it('walks transitively', () => {
    expect(recursiveConsumers(graph, '@aivastra/types')).toEqual([
      '@aivastra/api',
      '@aivastra/db',
      '@aivastra/web',
    ]);
  ...
  it('returns direct consumers for a leaf-adjacent package', () => {
    expect(recursiveConsumers(graph, '@aivastra/db')).toEqual(['@aivastra/api']);
  });

  it('returns nothing for a package nobody depends on', () => {
    expect(recursiveConsumers(graph, '@aivastra/api')).toEqual([]);
  });
});
...
describe('real repository workspace', () => {
  ...
  it('reproduces the documented shared-package impact table', () => {
    ...
    expect(consumersAsTargets('@aivastra/observability')).toEqual([
      'api',
      'chatbot',
      'dispatcher',
    ]);
...
export function readWorkspaceMembers(
  ...
  for (const path of manifestPaths) {
    ...
    const workspaceDeps = Object.entries(deps)
      .filter(([name, range]) => name.startsWith('@aivastra/') && String(range).startsWith('workspace:'))
      .map(([name]) => name)
      .sort();
...
describe('reasons', () => {
  it('records why each service was selected', () => {
    ...
    expect(result.reasons['@aivastra/types']).toEqual(['packages/types/src/jobs.ts']);
...
const baseResult: DetectResult = {
  ...
  changedPackages: ['@aivastra/api'],
  affectedPackages: ['@aivastra/api'],
  services: ['api'],
  testTargets: ['@aivastra/api'],
  migrationChanged: false,
  deploymentBundleChanged: false,
  infrastructureChanged: false,
  ciChanged: false,
...
describe('outputLines', () => {
  it('emits JSON arrays for matrix inputs and booleans as strings', () => {
    ...
    expect(lines).toContain('test_targets=["@aivastra/api"]');
  ...
  typecheck:
    ...
    steps:
      ...
      - name: Typecheck workspace
        run: pnpm -r --filter "!@aivastra/admin-mobile" run typecheck
...
Expected: `aivastra-prod-postgres`, `aivastra-prod-redis`, and `aivastra-prod-minio` show their **pre-deploy** container IDs and creation times. Only the deployed services show new IDs. If any stateful container was recreated, the `--no-deps` flag or the service scoping is wrong — stop and fix before proceeding to Phase 2.
...
describe('applyManualOverride with FORCE_ALL', () => {
  ...
  it('selects every testable package', () => {
    expect(forced().testTargets).toEqual(['@aivastra/api', '@aivastra/dispatcher']);
  });
...
| Serialize production deployments | Task 7 Step 2 (`concurrency: aivastra-production`, job-level) |
