...
**Tech Stack:** Fastify 5 + Zod (apps/api), Drizzle ORM/Postgres (packages/db), `pdfkit` (new dependency), R2/MinIO via `@aivastra/storage`, Resend (email), Next.js 15 + React Query (apps/catalogues-web), Vite + React (apps/admin-web).

## Global Constraints

- Package manager: pnpm workspaces only.
- ESM only (`"type": "module"`) everywhere.
- No `console.log` in committed code — use `@aivastra/logger` child loggers.
...
(Add it alongside whatever `@aivastra/types` import already exists at the top of the file — if there isn't one yet, add a new `import { Gstin } from '@aivastra/types';` line near the other imports.)
...
curl -X PATCH http://localhost:4000/admin/config \
  ...
  -d '{"seller":{"gstin":"27AAPFU0939F1ZV","legalName":"Aivastra Technologies Pvt Ltd","address":"123 Example St, Mumbai"}}'
...
describe('renderInvoicePdf', () => {
  it('produces a non-empty PDF buffer starting with the %PDF magic bytes', async () => {
    const buf = await renderInvoicePdf({
      ...
      seller: { gstin: '27AAPFU0939F1ZV', legalName: 'Aivastra Technologies Pvt Ltd', address: '123 Example St' },
...
Resolve any conflicts (unlikely — this is new code, not modifying anything else changed on `main` recently), rebuild affected packages (`pnpm --filter @aivastra/db build`, etc., same as the flat-saree case needed), re-run the full test suite against `main`'s codebase, then push and open a `main`-based PR the same way.
