...
- **ESM only**, pnpm workspaces, pino via `@aivastra/logger`, ASCII quotes, no `console.log` in committed code.
...
Run: `pnpm --filter @aivastra/db build && pnpm --filter @aivastra/api typecheck && pnpm --filter @aivastra/api test`
...
Run: `pnpm --filter @aivastra/api test -- funnel-rules`
...
Run: `pnpm --filter @aivastra/api test -- shopify-funnel-templates-admin`
...
Run: `pnpm --filter @aivastra/api test -- shopify-funnel-routes`
Expected: FAIL — 404, routes don't exist yet.
...
Run: `pnpm --filter @aivastra/dispatcher test -- shopify`
...
Add the import if not already present at the top of `apps/dispatcher/src/job/processor.ts`: confirm `schema` is already imported from `@aivastra/db` (it is, used throughout this file) — no new import needed since `shopifyProductGarments`/`shopifyFunnelTemplates` are accessed via the existing `schema.*` namespace import.
...
Run: `pnpm --filter @aivastra/dispatcher test && pnpm --filter @aivastra/dispatcher typecheck`
