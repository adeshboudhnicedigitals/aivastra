...
- Every `pnpm --filter <pkg> typecheck` / `pnpm --filter <pkg> test` command in this plan must be run from the repo root (`C:\Users\prave\OneDrive\Desktop\aivastra`).
...
**Interfaces:**
- Produces: `JOB_SOURCE` (const object, 13 keys), `JobSource` (type), `jobSourceSchema` (Zod enum), `LEGACY_JOB_SOURCE` (const object, 1 key: `API`), `WORKER_POOL` (const object, 5 keys), `WorkerPool` (type), `workerPoolSchema` (Zod enum) — all exported from `@aivastra/types`. Every later task consumes these.
...
Run: `pnpm --filter @aivastra/db typecheck`
...
Run: `pnpm --filter @aivastra/api test -- admin-workers`
...
Run: `pnpm --filter @aivastra/dispatcher test`
...
Run: `pnpm --filter @aivastra/api test -- admin-job-taxonomy`
...
Add `WORKER_POOL` to the `@aivastra/types` import already added in Task 2:
...
Run: `pnpm --filter @aivastra/admin dev` (and, separately, ensure `pnpm --filter @aivastra/api dev` and `pnpm docker:up` are running so the admin app has a live backend).
...
Run: `pnpm --filter @aivastra/api test -- jobs`
Expected: PASS (baseline).

- [ ] **Step 3: `create.ts` — import, typed `source`, 3 literals, 3 metrics**

Add `JOB_SOURCE` to the existing `@aivastra/types` import block (currently starting `import { ASPECT_DIMENSIONS, type CreateCatalogVideoJobRequest, ...`):
...
Run: `pnpm --filter @aivastra/api test -- merchant`
Run: `pnpm --filter @aivastra/api test -- shopify`
Expected: both PASS (baseline).

- [ ] **Step 2: `merchant/create-job.ts`**

Add `JOB_SOURCE` to the existing `@aivastra/types` import (line 3):
...
Add `JOB_SOURCE` to the existing `@aivastra/types` import (lines 3-7):
...
Run: `pnpm --filter @aivastra/api test -- admin-jobs-type`
Run: `pnpm --filter @aivastra/api test -- admin-credit-analysis`
Expected: both PASS (baseline).
...
Run: `pnpm --filter @aivastra/admin dev` (with the API and docker infra running).
...
Run: `pnpm --filter @aivastra/api test -- dev-read-routes`
Run: `pnpm --filter @aivastra/api test -- merchant-api-keys`
Run: `pnpm --filter @aivastra/api test -- dev-catalog`
...
Add `JOB_SOURCE, LEGACY_JOB_SOURCE` to the existing `@aivastra/types` import block, and `inArray` to the existing `drizzle-orm` import:
...
Add `JOB_SOURCE, LEGACY_JOB_SOURCE` to the existing `@aivastra/types` import:
...
In `apps/api/test/dev-read-routes.test.ts`, add the import (after `import { schema } from '@aivastra/db';`):
...
Run: `pnpm --filter @aivastra/db generate -- --custom --name=backfill_api_source_split`
...
Run: `pnpm --filter @aivastra/api test -- api-source-migration`
...
Run: `pnpm --filter @aivastra/api test` (the full suite — every test file's `startContainers()` call applies all migrations, including the new one, to a fresh DB; this confirms `0133` doesn't break migration application for any other test)
...
- [ ] Run `pnpm --filter @aivastra/api test` — full suite, PASS.
- [ ] Run `pnpm --filter @aivastra/dispatcher test` — full suite, PASS.
- [ ] Run `pnpm lint` — PASS on all touched files.
- [ ] Manual: with `pnpm docker:up`, `pnpm --filter @aivastra/api dev`, `pnpm --filter @aivastra/dispatcher dev`, and `pnpm --filter @aivastra/admin dev` all running — open the admin Workers page, add a worker with the `Merchant` job type checked, confirm it saves; open the admin Jobs page, confirm no `jobTypeBadge: unrecognized job source` warnings in the browser console for any existing job.
