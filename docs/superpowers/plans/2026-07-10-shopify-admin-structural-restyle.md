...
- No automated test harness exists for `apps/shopify` pages (matches the established pattern — no `.test.tsx` files for pages in this repo). Verification is `tsc -b` clean + a manual visual check via `pnpm --filter @aivastra/shopify-admin dev`, matching the prior plan's precedent exactly.
