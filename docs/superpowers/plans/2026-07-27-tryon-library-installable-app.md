...
**Tech Stack:** Fastify 5 + Zod (`@aivastra/types`), Drizzle ORM, `jose` JWTs, Next.js 15 App Router, React, Vitest integration tests.
...
In `apps/api/src/modules/auth/routes.ts`, change the `/v1/auth/login` route's schema (currently `schema: { body: LoginBody }`) to use the new type — first add `WebLoginBody` to the `@aivastra/types` import (currently line 3):
```ts
...
Run: `pnpm --filter @aivastra/web exec tsc --noEmit -p .`
Expected: no output.
...
> **Why this can't just be `lib/api.ts`:** that module holds its access token and a `BroadcastChannel('aivastra-auth')` at module scope, and on an unrecoverable 401 it hardcodes `window.location.href = '/login'` — the wrong login page for this route, and cross-talk risk with a regular session open in another tab. This is a from-scratch (but structurally identical) client scoped to this route.
