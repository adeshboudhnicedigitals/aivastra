...
**Goal:** Let `aivastra.com` (WordPress) "Buy Now" buttons link to `app.aivastra.com/pricing?plan=<slug>` and have the matching plan's Razorpay checkout open automatically, including when the visitor has to log in first.
...
Requires `pnpm docker:up` running and the API + web dev servers up (`pnpm dev`, or `pnpm --filter @aivastra/api dev` + `pnpm --filter @aivastra/web dev`).

Find a real, active plan slug (not `free`) in the local dev DB:

```bash
docker exec aivastra-postgres psql -U tryon -d tryon_dev -c "SELECT slug, is_active FROM credit_plans WHERE slug != 'free';"
```
