...
**Architecture:** `localStorage['aivastra_tryon_history']` already holds every past result, newest-first. Instead of a second `page--history` with small thumbnail-row cards, the `result` step renders that same array as full-size cards (a `<template>` element in the Liquid file, cloned per entry by JS). `showPage()`/the two-page model is deleted entirely — `showStep()` is the only navigation primitive left.
...
**Interfaces:**
- Produces (DOM structure Task 3's JS will query): `.aivastra-tryon__result-list` (empty container inside the `result` step), `.aivastra-tryon__result-empty` (empty-state paragraph), `.aivastra-tryon__result-card-template` (a `<template>` element whose content is one card: `.aivastra-tryon__result-card` wrapping `.aivastra-tryon__result-image`, `.aivastra-tryon__result-actions` (`.aivastra-tryon__add-to-cart` / `.aivastra-tryon__share`, each conditionally present per `cfg.behavior`), `.aivastra-tryon__cart-error`, `.aivastra-tryon__share-flash`, `.aivastra-tryon__view-cart`).
- Removes: `.aivastra-tryon__header-history`, `.aivastra-tryon__history-back`, `.aivastra-tryon__page--history`, `.aivastra-tryon__history-list`, `.aivastra-tryon__history-empty` — none of these are referenced by `WidgetPreview.tsx`, so removing them cannot break the drift test.

- [ ] **Step 1: Remove the header's History sub-header**

  Find (inside the `.aivastra-tryon__header` block):

  ```liquid
            <div class="aivastra-tryon__header-history" hidden>
              <button type="button" class="aivastra-tryon__history-back" aria-label="Back">
              ...
              <div>
                <p class="aivastra-tryon__heading">Your Try-Ons</p>
                <p class="aivastra-tryon__subheading">Results</p>
              </div>
            </div>
            <div class="aivastra-tryon__header-actions">
              <button type="button" class="aivastra-tryon__history-btn" aria-label="View history" hidden>
  ```
  ...
  ```liquid
            <div class="aivastra-tryon__header-actions">
              <button type="button" class="aivastra-tryon__history-btn" aria-label="View your try-ons" hidden>
  ```
...
- [ ] **Step 2: Replace the static Result step with an empty scrollable container**
  ...
  ```liquid
            <div class="aivastra-tryon__step aivastra-tryon__step--result" hidden>
              ...
              <div class="aivastra-tryon__result-actions">
                {%- unless cfg.behavior.addToCart == false -%}
                  <button
                    type="button"
                    class="aivastra-tryon__add-to-cart"
              ...
              <p class="aivastra-tryon__cart-error" hidden></p>
              <span class="aivastra-tryon__share-flash" hidden></span>
              <a class="aivastra-tryon__view-cart" href="/cart" hidden>View cart</a>
            </div>
          ...
          <div class="aivastra-tryon__page aivastra-tryon__page--history" hidden>
            <div class="aivastra-tryon__history-list"></div>
            <p class="aivastra-tryon__history-empty" hidden>
  ...
  ```liquid
            <div class="aivastra-tryon__step aivastra-tryon__step--result" hidden>
              <div class="aivastra-tryon__result-list"></div>
              <p class="aivastra-tryon__result-empty" hidden>
                No try-ons yet — results you generate will show up here.
              </p>
            </div>
          ...
          <template class="aivastra-tryon__result-card-template">
            <div class="aivastra-tryon__result-card">
              <img class="aivastra-tryon__result-image" alt="{{ 'tryon.result_alt' | t }}" />
...
- [ ] **Step 3: Verify the file is well-formed**
  ...
  Run: `pnpm --filter @aivastra/shopify-admin test -- widget-drift`

  Expected: all tests in `widget-drift.test.ts` PASS. This confirms every class `WidgetPreview.tsx` needs (`aivastra-tryon__step--result`, `aivastra-tryon__result-image`, `aivastra-tryon__result-actions`, `aivastra-tryon__add-to-cart`, `aivastra-tryon__share`, `aivastra-tryon__header-main`, etc.) still exists in the Liquid file, and the `cfg.behavior.addToCartLabel`/`shareLabel` defaults are unchanged.
...
**Interfaces:**
- Consumes: none (pure styling; Task 1's new class names `.aivastra-tryon__result-list` / `.aivastra-tryon__result-card` / `.aivastra-tryon__result-empty` are what this task styles).
- Produces: same visual language the old `.aivastra-tryon__history-*` rules gave the History page, now applied to `.aivastra-tryon__result-*`, plus a thin low-opacity divider between stacked cards (per product-owner decision).
...
- [ ] **Step 1: Drop the now-dead History header rules**
  ...
  ```css
  .aivastra-tryon__header-main[hidden],
  .aivastra-tryon__header-history[hidden] {
    display: none;
  }

  .aivastra-tryon__header-history {
    display: flex;
    align-items: center;
    gap: 10px;
  }

  .aivastra-tryon__history-back {
    display: flex;
    align-items: center;
  ...
  .aivastra-tryon__header-actions {
  ```
...
- [ ] **Step 2: Replace the History-card rules with Result-card rules**
  ...
  .aivastra-tryon__history-card {
    display: block;
  }

  .aivastra-tryon__history-media {
    position: relative;
    width: 100%;
    aspect-ratio: 3 / 4;
    border-radius: 16px;
  ...
  .aivastra-tryon__history-media img {
    width: 100%;
    height: 100%;
    object-fit: cover;
    display: block;
  ...
  .aivastra-tryon__history-meta strong {
    font-size: 14px;
    font-weight: 700;
    color: #111827;
  }

  .aivastra-tryon__history-meta span {
    color: #9ca3af;
    font-size: 11px;
    flex-shrink: 0;
  }

  .aivastra-tryon__history-actions {
    display: flex;
    justify-content: flex-end;
    gap: 8px;
  }

  .aivastra-tryon__history-share {
    width: 44px;
    height: 40px;
    border-radius: 10px;
  ...
  .aivastra-tryon__history-empty[hidden] {
    display: none;
  }
  ```
  ...
  .aivastra-tryon__result-card {
    display: block;
  }

  .aivastra-tryon__result-card + .aivastra-tryon__result-card {
    padding-top: 20px;
    border-top: 1px solid rgba(17, 24, 39, 0.08);
  }

  .aivastra-tryon__result-empty {
    flex: 1;
    display: flex;
    align-items: center;
    justify-content: center;
  ...
  .aivastra-tryon__result-empty[hidden] {
    display: none;
  }
  ```

  `.aivastra-tryon__result-image` and `.aivastra-tryon__result-actions` (further down the file) are untouched — they already render the full-width photo + action row today's single result used, and now apply identically inside every `.aivastra-tryon__result-card`.
...
- [ ] **Step 3: Let the Result step scroll instead of centering a single image**
  ...
  ```css
  .aivastra-tryon__step--pending,
  .aivastra-tryon__step--error {
    align-items: center;
    justify-content: center;
    text-align: center;
  }

  .aivastra-tryon__step--result {
    justify-content: center;
  }

  .aivastra-tryon__ready-preview {
  ```
  ...
  `.aivastra-tryon__step` (the base rule) already gives every step `flex: 1; min-height: 0; display: flex; flex-direction: column;`. Dropping the `justify-content: center` override means the Result step's cards stack from the top and `.aivastra-tryon__result-list`'s own `flex: 1; overflow-y: auto` handles the scrolling — same pattern the old `.aivastra-tryon__history-list` used.

- [ ] **Step 4: Verify**

  Run: `pnpm --filter @aivastra/shopify-admin test -- widget-drift`

  Expected: PASS. (This suite's only CSS-specific assertion is that the `.aivastra-tryon__retry` margin rule stays more specific than the preview's button reset — untouched by this task — plus the class-existence check from Task 1, which this task doesn't affect since it removes only classes `WidgetPreview.tsx` never references.)
...
**Interfaces:**
- Consumes: `.aivastra-tryon__result-list`, `.aivastra-tryon__result-empty`, `.aivastra-tryon__result-card-template` (Task 1), `.aivastra-tryon__result-card` and friends inside the template (Task 1).
- Produces (used by later steps in this same task): `renderResultList()` — re-reads `getHistory()` and repopulates `.aivastra-tryon__result-list`; `buildResultCard(entry)` — returns one populated `.aivastra-tryon__result-card` element, fully wired; `addVariantToCart(btn, errorEl, viewCartEl)` — adds the current page's variant to cart, updating only the three passed elements; `shareResult(url, flashEl)` and `flashShare(flashEl, message)` — now take the flash element to update instead of a module-level singleton.
