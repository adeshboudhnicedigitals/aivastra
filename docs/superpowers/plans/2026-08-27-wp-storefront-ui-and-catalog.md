...
Storefront child theme in Aivastra's brand colors, all 432 real garment
...
WooCommerce line):
```yaml
      - ./themes/storefront-aivastra:/var/www/html/wp-content/themes/storefront-aivastra
```
...
```yaml
  wpcli:
    ...
    volumes:
      ...
      - ../:/var/www/html/wp-content/plugins/aivastra-tryon
...
```bash
mkdir -p "D:/aivastra/webtool/wordpress-plugin/local-wp/themes/storefront-aivastra"
```

- [ ] **Step 3: Recreate the containers with the new mounts**

```bash
cd D:/aivastra/webtool/wordpress-plugin/local-wp
docker compose up -d
```
...
**Files:**
- Create: `local-wp/themes/storefront-aivastra/style.css`
- Create: `local-wp/themes/storefront-aivastra/functions.php`
...
- [ ] **Step 2: Create `local-wp/themes/storefront-aivastra/style.css`**

```css
/*
Theme Name: Storefront Aivastra
Template: storefront
Description: Storefront child theme skinned with Aivastra's brand palette for the demo storefront.
Version: 1.0.0
*/

:root {
  --aivastra-primary: #0f172a;
  --aivastra-primary-hover: #1e293b;
  --aivastra-accent: #6366f1;
  --aivastra-surface: #ffffff;
  --aivastra-surface-subtle: #f8fafc;
  --aivastra-border: #e2e8f0;
  --aivastra-text-main: #0f172a;
  --aivastra-text-muted: #64748b;
}

.main-navigation ul li a:hover,
.main-navigation ul li a:focus {
  color: var(--aivastra-accent);
}
...
button.button.alt,
input.button.alt {
  background-color: var(--aivastra-primary) !important;
  border-color: var(--aivastra-primary) !important;
  color: #fff !important;
  border-radius: 8px;
}
...
input.button.alt:hover {
  background-color: var(--aivastra-primary-hover) !important;
  border-color: var(--aivastra-primary-hover) !important;
}

/* Product grid cards (shop page, category archives) */
ul.products li.product {
  background: var(--aivastra-surface);
...
- [ ] **Step 3: Create `local-wp/themes/storefront-aivastra/functions.php`**
...
```bash
cd D:/aivastra/webtool/wordpress-plugin
docker run --rm -v "$(pwd):/app" -w /app php:8.2-cli php -l local-wp/themes/storefront-aivastra/functions.php
```
...
```bash
MSYS_NO_PATHCONV=1 docker exec local-wp-wpcli-1 wp theme activate storefront-aivastra --allow-root --path=/var/www/html
...
```bash
git add wordpress-plugin/local-wp/themes/storefront-aivastra/
git commit -m "feat(wordpress-plugin): add Storefront child theme in Aivastra brand colors"
```
...
manually, never `require`'d by `aivastra-tryon.php`.
 ...
 * _aivastra_import_source postmeta (format "{category-slug}::{filename}").
 *
 * Run with: wp eval-file wp-content/plugins/aivastra-tryon/local-wp/import-products.php
 */
...
function aivastra_already_imported(string $sourceTag): bool
{
    $existing = get_posts([
        'post_type' => 'product',
        'post_status' => 'any',
        'meta_key' => '_aivastra_import_source',
...
function aivastra_import_one(
    ...
    if (aivastra_already_imported($sourceTag)) {
        return false;
    }
    ...
    update_post_meta($productId, '_aivastra_import_source', $sourceTag);
...
$parentIds = [
    'Men' => aivastra_get_or_create_term('Men', 'men', 0),
    'Women' => aivastra_get_or_create_term('Women', 'women', 0),
];

$imported = 0;
$skipped = 0;
foreach (AIVASTRA_CATEGORY_MAP as $folder => $meta) {
    $termId = aivastra_get_or_create_term($meta['name'], $meta['slug'], $parentIds[$meta['parent']]);
    $dir = AIVASTRA_SOURCE_ROOTS[$meta['parent']] . '/' . $folder;
    if (!is_dir($dir)) {
    ...
    sort($files);
    $priceRange = AIVASTRA_PRICE_RANGES[$meta['slug']];

    $counter = 0;
    foreach ($files as $file) {
        $counter++;
        $created = aivastra_import_one(
            $dir . '/' . $file,
            $meta['slug'],
            $termId,
            $meta['name'],
...
```bash
MSYS_NO_PATHCONV=1 docker exec local-wp-wpcli-1 wp eval-file wp-content/plugins/aivastra-tryon/local-wp/import-products.php --allow-root --path=/var/www/html
```
...
```bash
MSYS_NO_PATHCONV=1 docker exec local-wp-wpcli-1 wp eval-file wp-content/plugins/aivastra-tryon/local-wp/configure-store.php --allow-root --path=/var/www/html
```
...
- [ ] **Step 1: Confirm the Aivastra plugin's category-mapping table now shows the real catalog**

Visit `http://localhost:8888/wp-admin/options-general.php?page=aivastra-tryon`
...
theme in Aivastra brand colors, 432 real garment products imported across 13
