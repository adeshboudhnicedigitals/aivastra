...
**Tech Stack:** Fastify 5, Drizzle ORM (Postgres), ioredis, Zod (`@aivastra/types`), Vitest (container-backed tests in `apps/api/test/`), React (admin-web).
...
- `apps/api/test/integration/queue-capacity.test.ts` — integration test through `/v1/jobs/tryon` (lives under `test/integration/` because it needs `app.inject` + full Studio fixture seeding, matching `jobs-create-looks.test.ts`'s existing pattern — run via `pnpm --filter @aivastra/api test:integration`, not the plain `test` command)
...
Expected: no errors; `psql` (or `pnpm --filter @aivastra/db exec drizzle-kit studio` if you prefer a UI) shows `merchants.job_rate_limit_per_min` as a nullable integer column.
...
**Interfaces:**
- Produces: `DEFAULT_JOB_RATE_LIMIT_PER_MIN` (number), `DEFAULT_MAX_QUEUE_DEPTH` (number) — both exported from `@aivastra/types`
...
```bash
pnpm --filter @aivastra/api exec vitest run test/job-rate-limit.test.ts
```
...
```bash
pnpm docker:up
pnpm --filter @aivastra/api exec vitest run test/dev-job-rate-limit.test.ts
```
...
```bash
pnpm --filter @aivastra/api exec vitest run test/dev-tryon-create.test.ts test/dev-saree-mannequin-create.test.ts test/dev-rate-limit.test.ts
```
...
```bash
pnpm --filter @aivastra/api exec vitest run test/queue-capacity-config.test.ts
```
...
```ts
import { DEFAULT_MAX_BATCH_JOBS, DEFAULT_MAX_QUEUE_DEPTH, PresignAppVideoBody, SystemConfigBody } from '@aivastra/types';
```
...
```bash
pnpm --filter @aivastra/api exec vitest run test/integration/queue-capacity.test.ts --config vitest.integration.config.ts
```
...
Expected: succeeds, including `@aivastra/types` and `@aivastra/db` picking up the new exports/column.
