...
- `admin-web` has no automated UI test suite. Verification per task is: `pnpm --filter @aivastra/admin typecheck`, `pnpm --filter @aivastra/admin build`, and a manual browser pass (open the relevant admin page, exercise every add/edit action touched by the task).
...
Run: `pnpm --filter @aivastra/admin typecheck && pnpm --filter @aivastra/admin build`
Expected: both succeed with no errors.

- [ ] **Step 5: Manual browser verification**

Run `pnpm --filter @aivastra/admin dev`, open Assets → Model Faces:
