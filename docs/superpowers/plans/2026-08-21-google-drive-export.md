...
export async function uploadFile(
  ...
  const boundary = `aivastra-${Date.now()}`;
...
export async function exportResultToDrive(
  ...
  const filename = `aivastra-${jobId.slice(0, 8)}.jpg`;
...
Run: `pnpm --filter @aivastra/web run typecheck` and
`pnpm --filter @aivastra/web run lint`
Expected: no errors.

- [ ] **Step 6: Manual verification**

Start the app (`pnpm --filter @aivastra/api dev` and
`pnpm --filter @aivastra/web dev`), generate a result in Studio, click
