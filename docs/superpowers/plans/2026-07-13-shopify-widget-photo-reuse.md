...
**Tech Stack:** Fastify 5 + Zod (`@aivastra/types`) on the backend, vanilla JS/Liquid/CSS in the Shopify theme app extension (no build step, no framework) — matching what's already there.
...
(replacing the current single-line `import { ShopifyCustomerJobRequest, ShopifyCustomerPresignRequest } from '@aivastra/types';` at line 3).
...
Run: `pnpm --filter @aivastra/api typecheck` — expect clean.
Run: `pnpm --filter @aivastra/api lint` — expect clean.
...
**Interfaces:**
- Produces: DOM elements with classes `.aivastra-tryon__reuse-panel`, `.aivastra-tryon__reuse-thumb`, `.aivastra-tryon__reuse-use`, `.aivastra-tryon__reuse-remove`, `.aivastra-tryon__upload-label-text`, `.aivastra-tryon__reuse-expired-note` — all consumed by Task 4's JS via `querySelector`.
...
```html
          <div class="aivastra-tryon__step aivastra-tryon__step--upload">
            <label class="aivastra-tryon__upload-btn">
              <img class="aivastra-tryon__upload-preview" hidden alt="" />
              <span class="aivastra-tryon__upload-placeholder">
...
```html
          <div class="aivastra-tryon__step aivastra-tryon__step--upload">
            <div class="aivastra-tryon__reuse-panel" hidden>
              <div class="aivastra-tryon__reuse-info">
                <img class="aivastra-tryon__reuse-thumb" alt="" />
                <button type="button" class="aivastra-tryon__reuse-use">Use this photo</button>
              </div>
              <button type="button" class="aivastra-tryon__reuse-remove">Not you? Remove</button>
            </div>
            <p class="aivastra-tryon__reuse-expired-note" hidden>
            ...
            <label class="aivastra-tryon__upload-btn">
              ...
              <span class="aivastra-tryon__upload-placeholder">
                <svg xmlns="http://www.w3.org/2000/svg" width="29" height="29" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v12"/><path d="m17 8-5-5-5 5"/><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/></svg>
                <span class="aivastra-tryon__upload-label-text">Drag and drop photo, or choose file</span>
              </span>
              <input type="file" accept="image/*" class="aivastra-tryon__file-input" />
            </label>
          </div>
...
Append to `tryon-widget.css` (after the last rule, `.aivastra-tryon__button`):

```css

.aivastra-tryon__reuse-panel {
  display: flex;
...
.aivastra-tryon__reuse-panel[hidden] {
  display: none;
}

.aivastra-tryon__reuse-info {
  display: flex;
  align-items: center;
  gap: 12px;
}

.aivastra-tryon__reuse-thumb {
  width: 48px;
  height: 48px;
...
.aivastra-tryon__reuse-expired-note {
  margin: 0 0 10px;
  font-size: 13px;
  color: #a94442;
}

.aivastra-tryon__reuse-expired-note[hidden] {
  display: none;
}
```
...
```bash
docker exec aivastra-redis redis-cli TTL "shopify:upload:<the r2Key from step 2>"
```
...
```bash
docker exec aivastra-redis redis-cli DEL "shopify:upload:<the r2Key>"
```
