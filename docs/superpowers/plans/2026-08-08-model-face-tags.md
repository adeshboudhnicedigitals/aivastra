...
Run: `pnpm --filter @aivastra/api test test/integration/model-face-tags.test.ts`
...
Run: `pnpm --filter @aivastra/api test test/integration/model-faces-tags-public.test.ts`
...
Run: `pnpm --filter @aivastra/admin dev`, open the admin panel, go to the Faces tab, edit a face, add tags like `warm tone, closeup`, save, re-open the edit modal, and confirm the tags persisted (comma-joined in the input).
...
Run: `pnpm dev` (or `pnpm --filter @aivastra/web dev` + `pnpm --filter @aivastra/api dev`), open Studio, tag two or more faces with different tags via the admin panel (Task 5) first, then in Studio step 1 click "View All" on the model picker and confirm:
