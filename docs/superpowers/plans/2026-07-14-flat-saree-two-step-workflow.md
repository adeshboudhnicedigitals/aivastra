...
**Tech Stack:** Fastify 5 + Zod (`@aivastra/types`), Drizzle ORM/Postgres, Redis Streams, Next.js 15 (studio wizard), Vite/React admin panel, Vitest (integration tests against the shared docker-compose Postgres/Redis/MinIO — no testcontainers).
...
- Integration tests reuse the shared docker-compose Postgres/Redis/MinIO (`pnpm docker:up` must be running) — no testcontainers. `apps/api/vitest.config.ts` excludes `test/integration/**` from the default `pnpm --filter @aivastra/api test` run; run integration tests with an override config (Task 6's Step 2 shows the exact command).
...
- `@aivastra/db` exports `* as schema` from `packages/db/src/index.ts` — never add a duplicate `schema` re-export.
...
- Generated: `packages/db/src/migrations/0106_<auto>.sql`, `packages/db/src/migrations/meta/0106_snapshot.json`, `packages/db/src/migrations/meta/_journal.json` (all auto-generated by `pnpm db:generate` — do not hand-edit)
...
Expected: creates `packages/db/src/migrations/0106_<random-name>.sql` containing three `ALTER TABLE "garment_subcategories" ADD COLUMN ...` statements, updates `meta/_journal.json` (new entry `idx: 106`) and `meta/0106_snapshot.json`.
...
```bash
git add packages/db/src/schema/models.ts packages/db/src/migrations/0106_*.sql packages/db/src/migrations/meta/_journal.json packages/db/src/migrations/meta/0106_snapshot.json
...
Run: `pnpm --filter @aivastra/types typecheck` — expect clean.
Run: `pnpm --filter @aivastra/types build` — expect success.
...
**Interfaces:**
- Consumes: `createSareeMannequinJob` from `./createSareeMannequin.js` (Task 6), `CreateSareeMannequinJobRequest` from `@aivastra/types` (Task 5).
...
Run: `pnpm --filter @aivastra/api typecheck` — expect clean (confirms the select shape still matches the route's inferred response type).
...
Run: `pnpm --filter @aivastra/dispatcher typecheck` — expect clean.
Run: `pnpm --filter @aivastra/dispatcher lint` — expect clean.
...
Run: `pnpm --filter @aivastra/dispatcher test -- saree-step2-workflow-override`
...
Run: `pnpm --filter @aivastra/dispatcher test -- happy-path`
...
Run: `pnpm --filter @aivastra/admin-web dev`, open the admin panel, go to Assets → Garment Types, edit a garment type. Confirm the new "Two-Step Generation" card appears, the switch toggles the two pickers, and saving persists (reload the page, confirm the switch state and picker selections survived).
...
Run: `pnpm --filter @aivastra/admin-web typecheck` — expect clean.
...
import { schema } from '@aivastra/db';
import { keys } from '@aivastra/storage';
import { CreateSareeMannequinJobRequest, CreateSimpleTryonRequest, CreateTryOnJobRequest } from '@aivastra/types';
import { and, asc, desc, eq, isNotNull, sql } from 'drizzle-orm';
import type { FastifyInstance } from 'fastify';
import { z } from 'zod';
import { AppError } from '../../lib/errors.js';
...
Run: `pnpm --filter @aivastra/admin typecheck && pnpm --filter @aivastra/admin lint`
...
- Generated: `packages/db/src/migrations/0107_<auto>.sql`, `packages/db/src/migrations/meta/0107_snapshot.json`, `packages/db/src/migrations/meta/_journal.json`
...
Run: `pnpm db:generate`
Expected: creates `packages/db/src/migrations/0107_<random-name>.sql` containing `DROP TABLE "saree_settings";` (drizzle-kit will prompt about the table being dropped — confirm the drop, since it's intentional), updates the journal/snapshot.
...
Run: `pnpm --filter @aivastra/db typecheck` — expect clean.

- [ ] **Step 6: Commit**

```bash
git add packages/db/src/schema/index.ts packages/db/src/migrations/0107_*.sql packages/db/src/migrations/meta/_journal.json packages/db/src/migrations/meta/0107_snapshot.json
...
Run: `pnpm --filter @aivastra/storage typecheck` — expect clean.
Run: `pnpm --filter @aivastra/storage build` — expect success.
Run: `pnpm --filter @aivastra/api typecheck` — expect clean (confirms nothing in `apps/api` still imports the removed builders).
...
pnpm --filter @aivastra/dispatcher test -- happy-path saree
```
