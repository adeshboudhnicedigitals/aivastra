...
- No `console.log` in committed code — use `app.log` / `@aivastra/logger`.
...
Run: `pnpm --filter @aivastra/dispatcher test -- sweeper-held-release`
...
```bash
pnpm db:migrate
pnpm --filter @aivastra/dispatcher test -- sweeper-held-release
pnpm --filter @aivastra/dispatcher test -- sweeper-video-sla
```
...
Run: `pnpm --filter @aivastra/api test -- merchant-catalog-bulk-hold`
...
Run: `pnpm --filter @aivastra/api test -- merchant-catalog-generate`
Expected: PASS.
...
- Produces:
  ...
  - Zod schemas `AdminHeldJobsResponse` and `AdminHeldJobsReleaseResponse` from `@aivastra/types`. Task 6 consumes both.
...
Run: `pnpm --filter @aivastra/api test -- admin-held-jobs`
...
Run: `pnpm --filter @aivastra/api test -- merchant-catalog-reconcile-held`
...
Run: `pnpm --filter @aivastra/api test -- merchant-catalog`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api test -- merchant-catalog-publish-pending`
...
Run:
```bash
pnpm --filter @aivastra/api test -- merchant-catalog
pnpm --filter @aivastra/api test -- kiosk
```
Expected: PASS.
...
Run: `pnpm --filter @aivastra/admin dev`, log in, open **Operations → Held Batches**.
...
Then run `pnpm --filter @aivastra/web dev`, log in as a merchant, open a subcategory → Bulk Upload, keep **Catalogue Images** selected, add two photos, fill SKU + prices, and press "Add 2 to Catalogue".
