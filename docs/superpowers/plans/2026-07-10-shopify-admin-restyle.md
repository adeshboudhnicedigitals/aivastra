...
**Goal:** Apply the AiVastra brand gradient (pink `#f55c7a` → amber `#f6b553`) to `apps/shopify`'s existing screens via Polaris's CSS custom-property theming layer, plus two small backend additions (per-status product counts, a "connected since" date) the restyled Dashboard needs. Zero functional regressions.
...
- This is a UI-upgrade / visual-restyle project. The mock's literal HTML/markup (`AiVastra Shopify Admin Dashboard/AiVastra Admin.dc.html`) is a loose reference only — do NOT replicate its raw div/inline-style structure. Keep all existing Polaris components — restyle via CSS custom-property overrides, not component replacement.
...
- After API changes: `pnpm --filter @aivastra/api typecheck` must be clean.
...
```bash
cd /mnt/vol1/PycharmProjects/aivastra_v1/apps/api
...
```bash
cd /mnt/vol1/PycharmProjects/aivastra_v1
...
```bash
cd /mnt/vol1/PycharmProjects/aivastra_v1/apps/shopify
npx tsc -b
cd /mnt/vol1/PycharmProjects/aivastra_v1
...
- [ ] "Sync products now" and "Top up on aivastra.com" buttons render pink (brand primary).
