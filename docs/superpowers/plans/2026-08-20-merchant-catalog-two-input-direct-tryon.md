...
```
$ docker exec aivastra-postgres psql -U <user> -d <db> -t -c \
...
Run:
```bash
docker exec aivastra-postgres psql -U <POSTGRES_USER> -d <POSTGRES_DB> -t -c \
...
Run: `pnpm --filter @aivastra/types typecheck` (or `pnpm typecheck` from root if that package
...
Run `pnpm --filter @aivastra/admin dev`, open Assets → Garment Types → "saree" → Edit, set
...
Run `pnpm --filter @aivastra/web dev`, open Tryon → a two-input-capable subcategory (saree,
...
- `pnpm --filter @aivastra/api test:integration` green, including the new test from Task 3
...
- `pnpm typecheck` and `pnpm lint` clean repo-wide (remember `admin-web`'s typecheck script
  ...
  skips it).
- `pnpm --filter @aivastra/web typecheck` and the Task 8 manual browser walk both done.
