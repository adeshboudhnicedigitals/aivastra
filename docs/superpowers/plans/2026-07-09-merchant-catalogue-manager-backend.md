...
- **`apps/merchant-web` (resolved, no task needed):** verified its `CatalogContent.tsx` declares fully local TypeScript types (`MerchantCatalogGender`, `MerchantCatalogItem`) and never imports from `@aivastra/types`. The schema/type changes in this plan do **not** break its build. It stays functionally sidelined (already-broken merchant login, pre-existing and unrelated to this plan) but keeps compiling. No file in `apps/merchant-web` is touched by this plan.
...
```bash
docker exec -i aivastra-postgres psql -U tryon -d tryon_dev -c "\d merchant_catalog_items"
```
...
```bash
docker exec -i aivastra-postgres psql -U tryon -d tryon_dev -c "\d merchant_catalog_items" && docker exec -i aivastra-postgres psql -U tryon -d tryon_dev -c "\d merchant_catalog_subcategories" && docker exec -i aivastra-postgres psql -U tryon -d tryon_dev -c "\d garment_subcategories" | grep default_pose_id
```
