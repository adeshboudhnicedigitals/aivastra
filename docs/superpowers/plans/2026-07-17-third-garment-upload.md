...
- `pnpm docker:up` must be running before any `pnpm --filter @aivastra/api test` or dispatcher test.
...
```bash
pnpm --filter @aivastra/api test -- garment-type-third-upload
```
...
```bash
pnpm --filter @aivastra/api test -- admin-workflows
```
...
```bash
pnpm --filter @aivastra/api test -- garment-types-sort-order
```
...
```bash
pnpm --filter @aivastra/api test -- jobs-create-looks
```
...
```bash
pnpm --filter @aivastra/dispatcher typecheck 2>/dev/null || pnpm typecheck
```
...
```bash
pnpm --filter @aivastra/admin-web build
```
...
```bash
pnpm --filter @aivastra/admin-web dev
```
...
4. Query `job_inputs` for the created job and confirm `third_garment_key` is populated:
   ```bash
   docker exec aivastra-postgres psql -U tryon -d tryon_dev -c "SELECT third_garment_key FROM job_inputs ORDER BY job_id DESC LIMIT 1;"
   ```
...
4. `pnpm --filter @aivastra/api test` — full suite green, including the new/extended files from Tasks 2, 3, 4, 5.
5. `pnpm --filter @aivastra/dispatcher test` — full suite green (or no *new* failures beyond the pre-existing `happy-path.test.ts` schema drift flagged in Task 7).
