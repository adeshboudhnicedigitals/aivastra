...
**Goal:** Grant a one-time, admin-configurable number of free credits to a Shopify store's owner the first time the store links to an AiVastra account, independent of Shopify's own day-based `trialDays` billing trial.
...
Run: `pnpm --filter @aivastra/api test -- resolution-config`
...
Run: `pnpm --filter @aivastra/api test -- admin-config`
...
Run: `pnpm --filter @aivastra/api typecheck && pnpm --filter @aivastra/types typecheck`
Expected: no errors
...
Run: `pnpm --filter @aivastra/api test -- shopify-billing-sync`
...
`apps/api/test/shopify-store-account-link.test.ts` already imports `schema` from `@aivastra/db` and `eq` from `drizzle-orm` (lines 1-2) — no new imports needed.
...
Run: `pnpm --filter @aivastra/api test -- shopify-store-account-link`
...
No test step: `apps/admin-web` has no test suite in this repo (confirmed — no `.test.tsx` files exist under `apps/admin-web`). This task is verified via typecheck and a manual check with `pnpm --filter @aivastra/admin dev`.
...
```tsx
                <div style={{ marginTop: 24, marginBottom: 8 }}>
                  ...
                  <div className="setting-desc" style={{ marginBottom: 12 }}>
                    ...
                    an AiVastra account — before the merchant picks a paid plan. Independent of any
...
Run: `pnpm --filter @aivastra/admin dev`, open Settings, confirm the "Shopify Free Trial" field loads with the value from `GET /admin/config` (25 by default), edit it, click Save, reload the page, confirm the new value persists.
