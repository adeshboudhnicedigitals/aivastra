...
```bash
docker exec -it $(docker ps --filter "name=postgres" --format "{{.Names}}" | head -1) psql -U postgres -d aivastra -c "\d catalogue_templates" -c "\d catalogue_template_looks"
```
...
```bash
pnpm --filter @aivastra/api test -- catalogue-templates-admin
```
...
```bash
pnpm --filter @aivastra/api test -- catalogue-templates-public
```
...
Re-run `pnpm --filter @aivastra/api test -- catalogue-templates-admin` to confirm this addition passes too, then commit this backend addendum separately:
