...
- ESM only (`.js` extensions on relative imports), pino via `@aivastra/logger` (no `console.log`), Biome formatting enforced by lefthook pre-commit.
...
Run: `pnpm db:migrate && pnpm --filter @aivastra/db typecheck`
...
Run: `pnpm --filter @aivastra/api test -- shopper`
...
Run: `pnpm --filter @aivastra/api test -- store-day`
...
- Produces:
  - `ShopifyStoreSettingsPatch` Zod schema exported from `@aivastra/types`
...
Run: `pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts test/integration/shopify-settings.test.ts`
...
Run: `pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts test/integration/shopify-limits.test.ts`
...
Run: `pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts test/integration/shopify-customer.test.ts`
...
In `tryon-block.liquid`, add to the `<div class="aivastra-tryon" ...>` attribute
...
`<div class="aivastra-tryon__page aivastra-tryon__page--main">`, immediately
after the `--ready` step block:

```liquid
            <div class="aivastra-tryon__step aivastra-tryon__step--email" hidden>
              <h2 class="aivastra-tryon__upload-title">One quick thing</h2>
              <p class="aivastra-tryon__upload-lead aivastra-tryon__email-lead">
                Enter your email to continue.
              </p>
              <input
                type="email"
                class="aivastra-tryon__email-input"
                placeholder="you@example.com"
                autocomplete="email"
                required
              />
              <label class="aivastra-tryon__email-consent">
                <input type="checkbox" class="aivastra-tryon__email-consent-input" />
                <span>Email me offers and updates from this store.</span>
              </label>
              <p class="aivastra-tryon__email-error" hidden></p>
              <button type="button" class="aivastra-tryon__email-submit">Continue</button>
            </div>
```
...
```js
    steps.email = root.querySelector('.aivastra-tryon__step--email');
...
.aivastra-tryon__email-error {
  color: #b42318;
  font-size: 13px;
  margin: 0 0 10px;
}

.aivastra-tryon__email-submit {
  width: 100%;
  padding: 12px 16px;
  border: none;
  border-radius: var(--aivastra-border-radius, 4px);
  background: var(--aivastra-button-color, #000);
  color: var(--aivastra-text-color, #fff);
  font-size: 15px;
  font-weight: 600;
  cursor: pointer;
}
...
2. `runShopifyRetention` takes `Logger` from `@aivastra/logger`, and the test
   ...
   `@aivastra/logger` rather than widening the function's parameter type.
...
Run: `pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts test/integration/shopify-retention.test.ts`
...
Run: `pnpm --filter @aivastra/dispatcher build && pnpm --filter @aivastra/shopify-admin typecheck`
Expected: both clean.
...
Run: `pnpm --filter @aivastra/api test -- shopify-me`
...
Run: `pnpm --filter @aivastra/shopify-admin typecheck && pnpm --filter @aivastra/api typecheck`
Expected: both clean.
...
Run: `pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts test/integration/shopify-customer.test.ts test/integration/shopify-limits.test.ts test/integration/shopify-settings.test.ts test/integration/shopify-retention.test.ts`
