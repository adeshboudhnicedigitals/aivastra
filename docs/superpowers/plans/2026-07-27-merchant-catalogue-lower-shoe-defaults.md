...
**Tech Stack:** Fastify 5, Drizzle ORM, Zod (`@aivastra/types`), Vitest integration tests (real Postgres/Redis/MinIO via `apps/api/test/helpers/containers.ts`), React + Vite admin panel.
...
Run: `pnpm --filter @aivastra/admin-web typecheck`
...
Run `pnpm --filter @aivastra/admin-web dev`, open Settings, and confirm:
