...
- No `console.log`. Use `app.log` / `@aivastra/logger`.
...
3. Request the page with the header:
   ...
   curl -i http://localhost:3000/tryon-library-app \
     -H "X-Aivastra-Device-Token: <accessToken from step 2>"
   ```
...
5. Repeat step 3 with an obviously invalid token (e.g. `X-Aivastra-Device-Token: garbage`):
...
- `pnpm --filter @aivastra/api test`, `pnpm --filter @aivastra/api test:integration`, `pnpm typecheck`, and `pnpm lint` all clean.
- Manual verification transcript from Task 2 Step 3 recorded in the PR description or commit message body.
- `docs/progress.md` updated per `CLAUDE.md`'s progress-tracking convention: a dated entry noting the new `POST /v1/auth/catalog-app-device-exchange` endpoint and the `X-Aivastra-Device-Token` header contract, since the Android-side implementation (separate repo) will need this documented somewhere discoverable.
