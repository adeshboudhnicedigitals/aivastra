...
Run: `pnpm --filter @aivastra/api test -- dev-keys`
...
Run: `pnpm --filter @aivastra/api test -- dev-image-sniff`
...
Run: `pnpm --filter @aivastra/storage typecheck`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api test -- dev-api-auth`
...
Run: `pnpm --filter @aivastra/api test -- dev-rate-limit`
...
Extend the `@aivastra/types` import:
...
Run: `pnpm --filter @aivastra/api add @fastify/swagger @scalar/fastify-api-reference`
...
Run: `pnpm --filter @aivastra/api test -- dev-openapi`
...
Then run the app (`pnpm --filter @aivastra/web dev`), log in as a merchant, visit `/developers`, and confirm: create a key → full key shown once with the warning → reload → only the prefix is visible → revoke → the key disappears from the list.
...
Run: `pnpm typecheck && pnpm lint && pnpm --filter @aivastra/api test`
Expected: all pass.
