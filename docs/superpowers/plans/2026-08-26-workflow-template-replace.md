...
```bash
docker exec aivastra-postgres psql -U tryon -d tryon_dev -c "\d workflow_template_archives"
```
...
Run: `pnpm --filter @aivastra/dispatcher test:integration` (or the equivalent `vitest.integration.config.ts` invocation this package uses — check `package.json` scripts)
...
Run: `pnpm --filter @aivastra/dispatcher test`
Run: `pnpm --filter @aivastra/dispatcher test:integration`
Expected: all pass.
...
Run: `pnpm --filter @aivastra/api test:integration`
Expected: all pass.
...
```ts
import { ReplaceWorkflowBody } from '@aivastra/types';
```
(add to the existing `@aivastra/types` import block at the top, alongside `CreateWorkflowBody` etc.)
...
Run: `pnpm --filter @aivastra/admin dev`
