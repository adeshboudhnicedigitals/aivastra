...
Run: `pnpm --filter @aivastra/api test -- tryon` (adjust the `-t`/file filter to whatever matches `create.ts`'s existing coverage — inspect `apps/api/test/integration/` for the exact filename first: `grep -rl "jobs/tryon'" apps/api/test/integration/`)
...
Run: `pnpm --filter @aivastra/api test -- saree-mannequin-job`
...
**Interfaces:**
- Consumes: `ProcessorConfig` (`apps/dispatcher/src/job/processor.ts:41-49`), `keys.output` (`@aivastra/storage`), `transitionJob` (`apps/dispatcher/src/job/state.ts`).
...
Run: `pnpm --filter @aivastra/dispatcher test -- saree-step2-promoter`
...
Run: `pnpm --filter @aivastra/api typecheck && pnpm --filter @aivastra/admin typecheck`
Expected: PASS.
...
Run: `pnpm build && pnpm typecheck && pnpm --filter @aivastra/api test && pnpm --filter @aivastra/dispatcher test`
