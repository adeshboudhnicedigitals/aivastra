...
- `@aivastra/db` exports `* as schema` — do not add a duplicate `schema` re-export.
...
```bash
pnpm --filter @aivastra/api test -- garment-type-mannequin-fields
```
...
```bash
pnpm --filter @aivastra/api test -- tryon-two-input-detect
```
...
```bash
pnpm --filter @aivastra/api test -- saree-step1-workflow
```
...
```bash
pnpm --filter @aivastra/api test -- saree-mannequin-job
```
...
```bash
pnpm --filter @aivastra/dispatcher test -- saree-mannequin
```
