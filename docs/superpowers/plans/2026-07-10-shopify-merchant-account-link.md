...
**Goal:** Link a Shopify merchant's aivastra user account to their `shopify_stores` row (mandatory gate in the embedded admin), and remove Shopify-native billing entirely in favor of `app.aivastra.com/pricing`.
...
- Run `pnpm --filter @aivastra/db build` after every `schema.ts` edit before running typecheck elsewhere — stale `dist/` output causes phantom type errors in `apps/api`.
...
- [ ] **Step 2: Rebuild `@aivastra/db` and generate the migration**
...
```bash
pnpm db:migrate
docker exec -i aivastra-postgres psql -U tryon -d tryon_dev -c "\d shopify_stores" | grep -i owner
docker exec -i aivastra-postgres psql -U tryon -d tryon_dev -c "\dt shopify_plans"
```
...
```bash
pnpm --filter @aivastra/api test -- shopify-store-account-link
```
...
git commit -m "feat(api): add POST /v1/shopify/store/account/link to link a merchant's aivastra account"
```
...
```bash
pnpm --filter @aivastra/api test -- shopify-me
```
...
```bash
pnpm --filter @aivastra/api test -- me-shopify-store-flag
```
  ...
  return (
    <Page title="Link your aivastra account">
      <Card>
        <BlockStack gap="300">
          <Text as="p">
            To use AiVastra Try-On, link this store to your aivastra account. Billing and
            credits are managed on app.aivastra.com — nothing is charged through Shopify.
          </Text>
          {error && (
...
```tsx
          ...
          <Card>
            <BlockStack gap="200">
              ...
              <Button
                onClick={() => window.open('https://app.aivastra.com/pricing', '_blank', 'noopener')}
              >
