...
Run: `pnpm --filter @aivastra/types exec tsc --noEmit`
Expected: no errors.
...
Run: `pnpm --filter @aivastra/api exec vitest run --exclude 'test/integration/**' -t "uploadLimits"` (adjust the `--exclude` flag if the file you edited lives under `test/integration/` instead — in that case run `pnpm --filter @aivastra/api exec vitest run test/integration/<file>.test.ts -t "uploadLimits"`)
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api exec vitest run test/integration/merchant-catalog-generate.test.ts -t "admin-configured limit"`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api exec vitest run test/integration/uploads.test.ts`
...
Run: `pnpm --filter @aivastra/api exec vitest run test/integration/merchant-tryon.test.ts -t "admin-configured limit"`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api exec vitest run test/integration/kiosk-jobs.test.ts -t "admin-configured limit"`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api exec vitest run test/dev-tryon-create.test.ts test/dev-saree-mannequin-create.test.ts`
...
Run: `pnpm --filter @aivastra/api exec vitest run test/shopify-catalog-generate.test.ts -t "admin-configured Shopify catalogue limit"`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api exec vitest run test/integration/shopify-customer.test.ts -t "admin-configured limit"`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api exec vitest run test/shopify-products.test.ts -t "admin-configured limit"`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api exec vitest run test/integration/admin-bulk-import.test.ts`
Expected: both tests PASS.
...
Run: `pnpm --filter @aivastra/admin exec tsc -b`
Expected: no errors.

- [ ] **Step 6: Manual verification**

Run: `pnpm --filter @aivastra/admin dev`, log in as an admin, navigate to Settings, confirm:
...
Run:
```bash
pnpm --filter @aivastra/api exec vitest run \
