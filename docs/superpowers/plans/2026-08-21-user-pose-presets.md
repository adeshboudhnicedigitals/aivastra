...
Run: `pnpm --filter @aivastra/types typecheck` (or `pnpm typecheck` from
...
- Consumes: `GET/POST/DELETE /v1/pose-presets` (Task 3), `PosePreset` /
  `ListPosePresetsResult` types (Task 2, importable from `@aivastra/types`),
...
Run: `pnpm --filter @aivastra/web dev` (and `pnpm docker:up` /
`pnpm --filter @aivastra/api dev` if not already running)
