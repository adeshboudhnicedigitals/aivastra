...
```bash
pnpm --filter @aivastra/storage test
```
...
```bash
pnpm --filter @aivastra/types build && pnpm --filter @aivastra/api build && pnpm --filter @aivastra/admin build
```
...
- `schema.modelFaces`, `schema.modelBackgrounds`, `schema.modelPoses` all exported from `@aivastra/db` via `packages/db/src/schema/index.ts` ✅
