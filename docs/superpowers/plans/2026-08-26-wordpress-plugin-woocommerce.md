...
**Goal:** Build the `aivastra-tryon` WordPress/WooCommerce plugin per
...
- **Repo placement:** a separate repository (`aivastra-tryon-woocommerce`),
...
storefront button; a browser-side widget script calls the aivastra dev API
...
Expected: FAIL — `aivastra-tryon.php` does not exist yet.
...
```json
{
  "name": "aivastra/tryon-woocommerce",
  "description": "Aivastra Try-On for WooCommerce",
...
define('AIVASTRA_TRYON_VERSION', '0.1.0');
...
`aivastra-tryon.php` for real.

- [ ] **Step 6: Commit**

```bash
git add composer.json phpunit.xml.dist tests/php/bootstrap.php aivastra-tryon.php tests/php/PluginBootstrapTest.php
...
Expected: FAIL — class `Aivastra_Connection_Settings` does not exist.
...
Expected: FAIL — class `Aivastra_Connection_Service` does not exist.
...
Neither this class nor `Aivastra_Connection_Settings` (Task 2) is declared
...
(`Mockery::mock(Aivastra_Connection_Settings::class)` in the test below
...
final class WidgetConfigTest extends TestCase
{
    ...
    {
        $config = Aivastra_Widget_Config::build(42, 'Blue Kurta', 'https://example.com/kurta.jpg');
    ...
    {
        $config = Aivastra_Widget_Config::build(1, 'No Image Product', false);
 ...
 * (Aivastra_Widget_Loader) extracts id/title/image from WooCommerce, so this
 * builder has no WooCommerce dependency and needs no WordPress bootstrap to
 * test.
 */
class Aivastra_Widget_Config
{
...
final class SettingsPageSanitizeTest extends TestCase
{
    public function test_trims_whitespace_around_a_pasted_key(): void
    {
        ...
        $this->assertSame(
            ...
            Aivastra_Settings_Page::sanitize_key_input('  sk_live_' . str_repeat('a', 43) . '  ')
        );
    }

    public function test_rejects_a_value_not_matching_the_key_format(): void
    {
        $this->assertSame('', Aivastra_Settings_Page::sanitize_key_input('not-a-key'));
    }

    public function test_accepts_empty_string_unchanged(): void
    {
        $this->assertSame('', Aivastra_Settings_Page::sanitize_key_input(''));
    }
}
```
...
Expected: FAIL — class `Aivastra_Settings_Page` does not exist.
...
1. **Settings → Aivastra Try-On** appears in the admin menu.
...
No new automated test in this task — `Aivastra_Widget_Loader::init()` is a
thin WooCommerce-hook registration that calls the already-tested
`Aivastra_Widget_Config::build()`; the hook wiring itself needs a live
...
.aivastra-tryon-modal[hidden] {
  display: none;
}

.aivastra-tryon-modal-content {
  background: #fff;
  border-radius: 8px;
  padding: 24px;
  max-width: 420px;
...
2. Connect a real widget key (issued via the "Create WordPress Widget Key"
   ...
   `window.AivastraTryOn` in the browser console shows the correct
...
```javascript
(function () {
  var config = window.AivastraTryOn;
  if (!config || !config.widgetKey) return;

  var currentImage = config.productImage;
  var button = document.getElementById('aivastra-tryon-button');
  var modal = document.getElementById('aivastra-tryon-modal');
  ...
  if (variationForm && window.jQuery) {
    window.jQuery(variationForm).on('found_variation', function (event, variation) {
      currentImage = window.AivastraWidgetLogic.resolveVariationImage(config.productImage, variation);
    });
  ...
  function pollJob(jobId) {
    fetch(config.apiBase + '/v1/dev/jobs/' + jobId, {
      ...
      .then(function (result) {
        var classified = window.AivastraWidgetLogic.classifyJobResponse(result.status, result.body);
  ...
  modal.addEventListener('click', function (event) {
    ...
    if (event.target.id === 'aivastra-tryon-generate') {
      var fileInput = document.getElementById('aivastra-tryon-file');
...
pointed at a real (or staging) aivastra API:
...
rsync -a --exclude='.git' --exclude='tests' --exclude='vendor' \
  ...
  ./ /tmp/aivastra-tryon/
cd /tmp && zip -r aivastra-tryon.zip aivastra-tryon
```

Expected: a zip whose root contains `aivastra-tryon/aivastra-tryon.php`,
