...
**Tech Stack:** Fastify (api), Vitest + testcontainers (api tests), React 18 + TypeScript + Vite (admin SPA), jose (JWT verify), `@aivastra/db` Drizzle ORM
...
```bash
pnpm --filter @aivastra/api test -- admin-me
```
...
```bash
pnpm --filter @aivastra/admin build 2>&1 | grep -E "error|warning" | head -20
```
