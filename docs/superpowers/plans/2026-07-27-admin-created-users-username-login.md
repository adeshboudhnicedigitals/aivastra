...
**Tech Stack:** Fastify 5 + Zod (`@aivastra/types`), Drizzle ORM (`packages/db`), Postgres CHECK constraint, React (admin-web Vite SPA + catalogues-web Next.js), Vitest integration tests against real Postgres/Redis.
...
Run: `pnpm typecheck`
Expected: clean across every package (`@aivastra/db`, `@aivastra/types`, `@aivastra/api`, `@aivastra/web`, `@aivastra/admin`, etc.).
