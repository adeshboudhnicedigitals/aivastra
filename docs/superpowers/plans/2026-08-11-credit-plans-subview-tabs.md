...
- No test step in any task — `apps/admin-web` has no test suite in this repo. Every task is verified via `pnpm --filter @aivastra/admin build` (typecheck + bundle) and, in the final task, a manual dev-server check.
