...
- No `console.log` — use `createLogger` (`@aivastra/logger`) if logging is ever added (none of
  these tasks need new logging).
- ESM only, TypeScript 5.6, pnpm workspaces.
- Every task ends green on `pnpm --filter @aivastra/chatbot test` and `pnpm typecheck`.
...
Run: `pnpm --filter @aivastra/chatbot test -- models`
...
Run: `pnpm typecheck && pnpm --filter @aivastra/chatbot test`
Expected: ALL PASS.
