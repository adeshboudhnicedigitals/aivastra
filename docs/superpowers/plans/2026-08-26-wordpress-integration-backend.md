...
**Tech Stack:** Fastify 5, Drizzle ORM, Postgres, Zod (`@aivastra/types`),
...
Run (from repo root): `pnpm --filter @aivastra/api test -- api-keys-schema`
...
Run: `pnpm --filter @aivastra/api test -- api-keys-schema`
...
Run: `pnpm --filter @aivastra/api test -- dev-widget-scope`
...
Run: `pnpm --filter @aivastra/api test -- dev-widget-scope dev-api-auth`
...
Run: `pnpm --filter @aivastra/api test -- dev-widget-scope dev-tryon-create dev-openapi`
...
and add `schema` and `eq` imports from `@aivastra/db` / `drizzle-orm`):
...
Run: `pnpm --filter @aivastra/api test -- dev-widget-scope dev-tryon-create`
Expected: PASS
...
import { DEV_WIDGET_KEY_RATE_LIMIT_PER_MIN } from '@aivastra/types';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';
...
Run: `pnpm --filter @aivastra/api test -- widget-key-rate-limit`
Expected: FAIL — module `../src/lib/widget-key-rate-limit.js` does not exist,
and `DEV_WIDGET_KEY_RATE_LIMIT_PER_MIN` is not exported from `@aivastra/types`.
...
Run `pnpm --filter @aivastra/web dev`, log in as a merchant with API access
