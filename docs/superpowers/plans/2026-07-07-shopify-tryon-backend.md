...
- **Conventions:** ESM only (`.js` import specifiers), pnpm workspaces, pino via `@aivastra/logger` (no `console.log`), Postgres/Redis bind `127.0.0.1`. `@aivastra/db` exports `* as schema` — never add a duplicate re-export.
...
Run: `pnpm --filter @aivastra/db typecheck && pnpm --filter @aivastra/db build`
...
Run: `pnpm --filter @aivastra/api test -- shopify-crypto`
...
Run: `pnpm --filter @aivastra/api test -- shopify-service`
...
Run: `pnpm --filter @aivastra/api test -- shopify-plans`
...
Run: `pnpm --filter @aivastra/api test -- shopify-webhooks`
...
Make `garmentImageUrl` optional there too (Shopify path supplies no URL): change it to `.optional()`. Build the package: `pnpm --filter @aivastra/types build`.
...
Run: `pnpm --filter @aivastra/api test -- shopify-jobs`
Expected: PASS (2 tests). Also run the existing widget flow if a test exists: `pnpm --filter @aivastra/api test -- widget` to confirm no regression on the URL path.
...
Run: `pnpm --filter @aivastra/api typecheck && pnpm --filter @aivastra/dispatcher typecheck`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/api test -- shopify-billing`
Expected: FAIL — module missing.
...
Run: `pnpm docker:up && pnpm --filter @aivastra/api test`
