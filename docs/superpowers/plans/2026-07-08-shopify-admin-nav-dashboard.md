...
- **ESM only** (`.js` import specifiers in the backend), pnpm workspaces, pino via `@aivastra/logger`, ASCII quotes, no `console.log` in committed code.
- **Backend tasks are full TDD** (RED/GREEN evidence required). **Frontend tasks have no automated test harness** — verification is `pnpm --filter @aivastra/shopify-admin build` succeeding plus manual smoke-testing.
...
Run: `pnpm --filter @aivastra/api test -- shopify-onboarding`
...
1. Reload the embedded app in the already-installed dev store (`ai-vastra-store` or `aivastra`, whichever this session's ngrok tunnel is currently pointed at — repoint if needed per this session's earlier setup).
