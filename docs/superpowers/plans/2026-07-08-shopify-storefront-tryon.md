...
- **ESM only** (`.js` import specifiers), pnpm workspaces, pino via `@aivastra/logger`, ASCII quotes, no `console.log` in committed code.
...
Run: `pnpm --filter @aivastra/api test -- shopify-cors`
...
This requires two new imports at the top of `apps/api/src/server.ts` — add them alongside the existing `@aivastra/db` usage (check whether `schema`/`sql` are already imported elsewhere in this file; if not, add):
...
Run: `pnpm --filter @aivastra/api test -- widget-job-status` (or your extended existing file's name)
...
Run: `pnpm --filter @aivastra/api test -- widget-job-status`
Expected: PASS (2 tests).

- [ ] **Step 6: Run the full API suite + typecheck**

Run: `pnpm --filter @aivastra/api test && pnpm --filter @aivastra/api typecheck`
...
Run: `pnpm --filter @aivastra/api test -- shopify-metafields`
...
Run: `pnpm --filter @aivastra/api test -- shopify-oauth`
...
**Interfaces:**
- Consumes: `POST /v1/widget/presign`, `POST /v1/widget/jobs`, `GET /v1/widget/jobs/:id` (all existing, the last one now returning `resultUrl` per Task 2). All three require an `x-widget-key` header — this extension reads that value from `shop.metafields.aivastra.widget_key` (written by Task 3) at Liquid render time.
...
.aivastra-tryon__modal[hidden] {
  display: none;
}

.aivastra-tryon__modal-content {
  position: relative;
  background: #fff;
  border-radius: 8px;
  padding: 24px;
...
.aivastra-tryon__step[hidden] {
  display: none;
}

.aivastra-tryon__result-image {
  max-width: 100%;
  border-radius: 4px;
}
```
...
1. In the `ai-vastra-store` (or `aivastra`) dev store's theme editor, open the product template, add the "Try It On" app block, save.
...
3. Click it, upload a real photo, confirm: modal opens, upload succeeds, job creates (may hit the 202 "preparing" path if that product was never synced with an active garment — check `shopify_product_garments` for that product's `status` first via the same `docker exec aivastra-postgres psql` pattern used throughout this session, and if `active`, expect a real job to create and poll through to a result or failure, depending on whether a `workflowTemplateId` + `'shopify'`-capable worker are configured, per the existing operational prerequisites from Task 10).
