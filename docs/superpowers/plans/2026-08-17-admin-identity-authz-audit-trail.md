...
`@aivastra/observability` (Prometheus registry).
...
- `packages/db/src/index.ts` exports `* as schema` — never add a duplicate `schema`
  re-export; import `@aivastra/db` as `workspace:*`.
...
- Integration tests: `apps/api/test/integration/**`, run via
  `pnpm --filter @aivastra/api test:integration` against the docker-compose
...
- [x] **Alerting on audit-insert failure — concrete mechanism, not just "must be
      ...
      `@aivastra/observability`'s shared registry) in the error path, *before*
...
- Integration tests in `apps/api/test/integration/`, run via
  `pnpm --filter @aivastra/api test:integration` against the docker-compose Postgres
