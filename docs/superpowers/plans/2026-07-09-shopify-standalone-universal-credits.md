...
- `packages/db` must be rebuilt (`pnpm --filter @aivastra/db build`) after schema changes before `pnpm typecheck` will pick them up (learned earlier this session — stale `dist/` causes phantom type errors in `apps/api`).
...
- [ ] **Step 5: Run migrations, rebuild `@aivastra/db`, confirm**
...
Run: `pnpm --filter @aivastra/api test -- shopify-widget-auth -t "missing"`
...
Run: `pnpm --filter @aivastra/api test -- customer-auth`
Expected: FAIL — `Cannot find module './customer-auth.js'`. (If `ioredis-mock` isn't installed, run `pnpm --filter @aivastra/api add -D ioredis-mock` first.)
...
Run: `pnpm --filter @aivastra/api typecheck && pnpm --filter @aivastra/api test -- webhook`
...
Run: `pnpm --filter @aivastra/api typecheck && pnpm --filter @aivastra/api test -- billing`
...
Run: `pnpm --filter @aivastra/api test && pnpm --filter @aivastra/dispatcher test`
Expected: all pass.
...
7. On `INSUFFICIENT_CREDITS` (402) from the `jobs` call, show a distinct message: "Out of credits — top up your account" linking to `https://app.aivastra.com/pricing`.
...
```liquid
        <div class="aivastra-tryon__step aivastra-tryon__step--signin">
          <p>Sign in to try this on with your own credits.</p>
          <button type="button" class="aivastra-tryon__signin">Sign in to try on</button>
        </div>
```
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        window.opener.postMessage({ type: 'aivastra-widget-link', code, nonce }, origin);
        setStatus('done');
...
Run `pnpm --filter @aivastra/web dev`, open `http://localhost:3000/widget-link-complete?origin=http://localhost:3000&nonce=test` directly in a browser while logged in, confirm it calls the link endpoint and shows "Linked!".
