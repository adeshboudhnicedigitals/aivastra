...
Run: `pnpm --filter @aivastra/db test -- garment-taxonomy`
...
Run: `pnpm --filter @aivastra/db test`
...
Run: `pnpm build`
Expected: succeeds — confirms nothing downstream (`apps/api`, `apps/dispatcher`, etc.) breaks by importing the updated `@aivastra/db` schema, since Phase 1 is purely additive and no existing code references the new tables/columns yet.
...
- [ ] `pnpm --filter @aivastra/db test -- garment-taxonomy` passes (12 tests).
...
- [ ] `pnpm build` succeeds at the repo root, confirming no downstream package broke from the `@aivastra/db` schema change.
