...
- `pnpm --filter @aivastra/web dev` must be running to manually verify each task in the browser per CLAUDE.md's UI-testing requirement.
...
With `pnpm --filter @aivastra/web dev` running, open `/studio`, resize the browser window, confirm the Garment Type row still shows the same number of cards as before (no visible behavior change).
...
With `pnpm --filter @aivastra/web dev` running, verify each item from `docs/superpowers/specs/2026-06-17-studio-single-page-design.md`'s Testing section:
...
Run: `pnpm --filter @aivastra/web typecheck && pnpm --filter @aivastra/web lint`
