...
Expected: migration applies with no errors; `psql` or a quick `pnpm --filter @aivastra/api test` run against an unrelated existing test confirms the schema loads.
...
Run: `pnpm --filter @aivastra/api test -- partner-client`
...
**Interfaces:**
- Consumes: `getActiveSubscription` from `./partner-client.js` (Task 3), `creditsForPlanHandle` from `./billing-plans.js` (Task 4), `DB` from `@aivastra/db`, `schema` from `@aivastra/db`.
- Produces:
...
Run: `pnpm --filter @aivastra/api test -- shopify-billing-scheduler`
Expected: FAIL — module not found
...
Run: `pnpm --filter @aivastra/api test -- shopify-billing-routes`
Expected: FAIL — module not found
...
Run: `pnpm --filter @aivastra/api test -- me-shopify-store-flag`
...
Run: `pnpm --filter @aivastra/api test -- webhook`
...
Run: `pnpm --filter @aivastra/shopify-admin test -- billing`
Expected: FAIL — module not found
...
Expected: PASS — check `apps/shopify/src/__tests__/widget-design-loading.test.ts` and any Dashboard-related snapshot/DOM test isn't asserting on the old button text; update it if so (search first: `grep -rn "Top up on aivastra" apps/shopify/src/__tests__`).
