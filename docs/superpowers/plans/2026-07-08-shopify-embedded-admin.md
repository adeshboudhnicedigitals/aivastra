...
- **ESM only** (`.js` import specifiers in the backend; ESM throughout), pnpm workspaces, pino via `@aivastra/logger`, ASCII quotes, no `console.log` in committed code.
- **Backend tasks are full TDD** (RED/GREEN evidence required). **Frontend tasks have no automated test harness** — matches `apps/admin-web`'s existing precedent (it has none either) — verification is `pnpm --filter @aivastra/shopify-admin build` succeeding plus manual smoke-testing against the real dev store.
...
Run: `pnpm --filter @aivastra/api test -- shopify-sync`
...
Run: `pnpm --filter @aivastra/api test -- shopify-products`
...
Run: `pnpm --filter @aivastra/api test -- shopify-products` — expect the existing 3 tests still PASS (pure refactor, no behavior change) before continuing.
...
Run: `pnpm --filter @aivastra/api test -- shopify-jobs`
...
**No TDD** — this app has no automated test harness (matches `apps/admin-web`'s existing precedent). Verification is `pnpm --filter @aivastra/shopify-admin build` succeeding, plus manual smoke-testing against the real dev store once deployed (last task of this plan covers the full manual verification pass).
...
2. Run `pnpm --filter @aivastra/shopify-admin dev` (or deploy a built version behind the same ngrok tunnel already used this session).
