...
Today `infra/observability/alloy.alloy` keeps every container matching `/aivastra-.*` off the shared Docker socket. Once a second stack runs on the same host, each environment's Alloy ingests the other's logs. This task makes the filter env-driven, defaulting to current behaviour so the local `docker-compose.yml` stack is unaffected.
...
**Interfaces:**
- Consumes: nothing.
- Produces: environment variable `ALLOY_CONTAINER_REGEX` (string, a Go RE2 regex matched against `__meta_docker_container_name`, which is the container name with a leading `/`). Task 2's staging compose sets it to `/aivastra-staging-.*`.
...
Expected: the `keep` rule has a hardcoded `regex = "/aivastra-.*"`, and the `service` rule strips an optional `prod-` segment only. Both are what the next step replaces.

- [ ] **Step 2: Widen the service-label regex and make the keep filter env-driven**

Edit `infra/observability/alloy.alloy`. Replace the `discovery.relabel "aivastra"` block with:

```river
// Keep only this stack's containers and derive a clean `service` label from the
// container name. Matches prod (aivastra-prod-api), staging (aivastra-staging-api)
// and local (aivastra-api), stripping the optional environment segment so `service`
...
discovery.relabel "aivastra" {
  targets = discovery.docker.aivastra.targets
  ...
  rule {
    source_labels = ["__meta_docker_container_name"]
    regex         = coalesce(sys.env("ALLOY_CONTAINER_REGEX"), "/aivastra-.*")
    action        = "keep"
  }
}
```
...
```yaml
      ALLOY_CONTAINER_REGEX: /aivastra-prod-.*
```
...
import re
cases = {
  "/aivastra-prod-api": "prod",
  "/aivastra-prod-dispatcher": "prod",
  "/aivastra-staging-api": "staging",
  "/aivastra-api": "local",
}
prod_keep = re.compile(r"/aivastra-prod-.*$")
stag_keep = re.compile(r"/aivastra-staging-.*$")
svc = re.compile(r"/aivastra-(?:prod-|staging-)?(.*)$")
...
Expected output — prod containers kept only by prod, staging only by staging, `aivastra-api` kept by neither scoped filter (it is a local-only name), and every `service` label reduced to the bare service name:

```
/aivastra-prod-api               prod_keep=True  staging_keep=False service=api
/aivastra-prod-dispatcher        prod_keep=True  staging_keep=False service=dispatcher
/aivastra-staging-api            prod_keep=False staging_keep=True  service=api
/aivastra-api                    prod_keep=False staging_keep=False service=api
...
Expected: the line `ALLOY_CONTAINER_REGEX: /aivastra-prod-.*` appears under the alloy service. If `docker compose config` errors on missing variables, that is expected for the example env file — in that case run `docker compose -f infra/docker-compose.prod.yml config --quiet` and confirm it reports no syntax error, then `grep -n 'ALLOY_CONTAINER_REGEX' infra/docker-compose.prod.yml` to confirm placement.
...
- Produces: compose project `aivastra-staging` with services `postgres redis minio minio-bootstrap api chatbot dispatcher web admin shopify-admin alloy`; container names `aivastra-staging-<service>`; the env file path `.env.staging` relative to the repo root, mounted into `api` and `chatbot` at `/app/.env`. Task 3's sync script and Task 4's guardrail both address containers by these names.
...
# `pgdata` here materialises as `aivastra-staging_pgdata` and cannot collide with
# prod's `aivastra-prod_pgdata`.
#
...
#   staging-app.aivastra.com/         -> localhost:3100  (Next.js web)
#   staging-app.aivastra.com/v1/      -> localhost:4100  (Fastify API)
#   staging-app.aivastra.com/minio/   -> localhost:9100  (MinIO pass-through)
#   staging-admin.aivastra.com/       -> localhost:3101  (Admin SPA)
...
services:
  postgres:
    ...
    container_name: aivastra-staging-postgres
    ...
    healthcheck:
      ...
      retries: 10
    networks:
      - aivastra-staging-net

  redis:
    image: redis:7-alpine
    container_name: aivastra-staging-redis
    restart: unless-stopped
  ...
  minio:
    ...
    container_name: aivastra-staging-minio
    restart: unless-stopped
  ...
  minio-bootstrap:
    ...
    container_name: aivastra-staging-minio-bootstrap
    restart: "no"
    depends_on:
      minio:
  ...
  api:
    ...
    container_name: aivastra-staging-api
    restart: unless-stopped
    ports:
      - "127.0.0.1:4100:4000"
    volumes:
    ...
    command: >
      sh -c "
        ...
        pnpm --filter @aivastra/api start
      "
    depends_on:
      postgres:
        condition: service_healthy
  ...
  chatbot:
    ...
    container_name: aivastra-staging-chatbot
    restart: unless-stopped
    ports:
      - "127.0.0.1:4300:4200"
    volumes:
  ...
  dispatcher:
    ...
    container_name: aivastra-staging-dispatcher
    restart: unless-stopped
    env_file:
      - ../.env.staging
    depends_on:
  ...
  web:
    ...
    container_name: aivastra-staging-web
    restart: unless-stopped
    environment:
      NEXT_PUBLIC_BASE_PATH: ${NEXT_PUBLIC_BASE_PATH:-}
      NEXT_PUBLIC_API_URL: ${NEXT_PUBLIC_API_URL}
  ...
  admin:
    ...
    container_name: aivastra-staging-admin
    restart: unless-stopped
    ports:
      - "127.0.0.1:3101:80"
    networks:
  ...
  shopify-admin:
    build:
      ...
      args:
        ...
        VITE_AIVASTRA_APP_URL: ${VITE_AIVASTRA_APP_URL:-https://staging-app.aivastra.com}
        VITE_API_BASE_URL: ${VITE_API_BASE_URL:-https://staging-app.aivastra.com}
    container_name: aivastra-staging-shopify-admin
    restart: unless-stopped
    ports:
      - "127.0.0.1:3103:80"
    networks:
  ...
  alloy:
    ...
    container_name: aivastra-staging-alloy
    restart: unless-stopped
    command:
      - run
    ...
    environment:
      ...
      # matching /aivastra-prod-.* filter. Both read the same Docker socket.
      ALLOY_CONTAINER_REGEX: /aivastra-staging-.*
...
# ─── Public URLs ──────────────────────────────────────────────────────────────
WEB_URL=https://staging-app.aivastra.com
...
Expected: names prefixed `aivastra-staging_` (for example `name: aivastra-staging_pgdata`). If they render as bare `pgdata`, the `name:` key at the top of the compose file is missing or misspelled.
...
names namespaced aivastra-staging-*, host ports at prod+100. SHOPIFY_TOKEN_ENC_KEY
...
good_env
sed -i '/^AIVASTRA_ENV=/d' "$TMP/staging.env"
check "missing AIVASTRA_ENV marker is rejected" 1
...
good_env
sed -i 's|^EMAIL_FROM=.*|EMAIL_FROM=no-reply@aivastra.com|' "$TMP/staging.env"
...
**Interfaces:**
- Consumes: container names `aivastra-prod-postgres`, `aivastra-staging-postgres`, `aivastra-prod-minio`, `aivastra-staging-minio` and the compose project from Task 2.
...
if [ "$DRY_RUN" -eq 1 ]; then
  echo "DRY-RUN: docker exec aivastra-prod-postgres pg_dump -Fc -U $PROD_PG_USER $PROD_PG_DB > $DUMP"
else
  docker exec aivastra-prod-postgres pg_dump -Fc -U "$PROD_PG_USER" "$PROD_PG_DB" > "$DUMP"
...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  docker exec aivastra-staging-postgres dropdb -U "$STAGING_PG_USER" --if-exists --force "$STAGING_PG_DB"
  docker exec aivastra-staging-postgres createdb -U "$STAGING_PG_USER" "$STAGING_PG_DB"
...
```bash
grep -nE 'aivastra-prod' scripts/staging/sync-from-prod.sh
```

Expected: exactly one line, the `pg_dump` in section 1. Every other prod reference must be a read (`$PROD_ENV` lookups, the `prodm` mirror source). If any line combines `aivastra-prod` with `dropdb`, `createdb`, `psql -f`, `mc mirror ... prodm/` as a *destination*, or `mc rm`, stop and fix it.
...
```yaml
    ...
    concurrency:
      ...
      group: aivastra-${{ github.ref == 'refs/heads/main' && 'production' || 'staging' }}
...
Add a `## Staging Environment` section after `## Adding a GPU worker`, stating: staging runs from `dev` on the same VPS as prod under the `aivastra-staging` compose project; ports are prod+100; data is an unscrubbed prod snapshot refreshed by `scripts/staging/sync-from-prod.sh`; `.env.staging` must pass `scripts/staging/check-staging-env.sh` or the deploy aborts; see `docs/staging-runbook.md`.
...
Expected: the run deploys only to `STAGING_DEPLOY_PATH`, and its concurrency group reads `aivastra-staging`. If the guardrail step fails, that is the guardrail working — fix `.env.staging` and push again.
...
```bash
docker ps --filter 'name=aivastra-prod-' --format '{{.Names}}\t{{.Status}}'
```
...
Open `https://staging-app.aivastra.com`, log in with an account from the snapshot.
...
In the production Grafana account, query `{job="aivastra"}` over the last 15 minutes and inspect the `container` label. Repeat in the staging account.

Expected: production shows only `aivastra-prod-*`; staging shows only `aivastra-staging-*`. Overlap means `ALLOY_CONTAINER_REGEX` is unset on one of the two Alloy containers — check with `docker exec aivastra-prod-alloy env | grep ALLOY_CONTAINER_REGEX`.
