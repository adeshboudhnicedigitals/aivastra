...
- Migration flow (mandatory order): edit schema TS → `pnpm --filter @aivastra/db build` → `pnpm db:generate` → `pnpm db:migrate`. The `local-db-sync` pre-push hook BLOCKS push if local `.sql` count > applied count, so always migrate before pushing.
...
- Admin UI: use `C`/CSS-var design tokens, no raw hex. Logger: pino via `@aivastra/logger`, no `console.log` in committed API code (admin SPA `console.error` is fine).
...
- Commit after each task. Run `pnpm --filter @aivastra/api typecheck` and the relevant package build/typecheck before each commit.
...
Run: `pnpm --filter @aivastra/api exec vitest run src/modules/admin/tryon-detect.test.ts`
...
Run: `pnpm --filter @aivastra/types build && pnpm --filter @aivastra/api typecheck`
Expected: no errors.
...
Run admin dev (`pnpm --filter @aivastra/admin dev`) + api dev. Open Workflows → Upload → pick "Tryon", choose `templates/tryonupper25062026 (2).json`, click Parse. Expected: person=1000, garment=1006, output=994, positive=1001:111, negative=1117 pre-filled; Create succeeds; new row appears with `workflowType: tryon`.
...
Run: `pnpm --filter @aivastra/storage build`
Expected: no errors.
...
Run: `pnpm --filter @aivastra/types build && pnpm --filter @aivastra/storage build && pnpm --filter @aivastra/api typecheck`
Expected: no errors.
