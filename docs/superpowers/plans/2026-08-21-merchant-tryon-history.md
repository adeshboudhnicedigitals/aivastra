...
**Interfaces:**
- Consumes: `app.db`, `schema.jobs` (from `@aivastra/db`), `app.requireMerchant` / `req.merchantClientId` (from `apps/api/src/plugins/portal-auth.ts`), `AppError` (from `apps/api/src/lib/errors.js`) — all already imported/available in `tryon.routes.ts`
- Produces: `MerchantTryonHistoryQuery`, `MerchantTryonHistoryDay`, `MerchantTryonHistoryResponse` (exported from `@aivastra/types`) — Task 3 (Android) mirrors this response shape by hand in Kotlin, so keep the JSON field names (`date`, `inputCount`, `generatedCount`, `failedCount`, `days`, `nextCursor`) exact.
...
**Files:**
- Create: `apps/virtual_tryon_android/app/src/main/java/aivastra/nice/interactive/data/models/TryOnHistoryModels.kt`
- Modify: `apps/virtual_tryon_android/app/src/main/java/aivastra/nice/interactive/api/ApiService.kt`
- Create: `apps/virtual_tryon_android/app/src/main/java/aivastra/nice/interactive/data/repository/TryOnHistoryRepository.kt`

**Interfaces:**
- Consumes: `ApiClient.apiService` (existing authenticated Retrofit client, `apps/virtual_tryon_android/app/src/main/java/aivastra/nice/interactive/api/ApiClient.kt`) — same bearer-token client every other repository in this app uses
...
In `apps/virtual_tryon_android/app/src/main/java/aivastra/nice/interactive/api/ApiService.kt`, add this import alongside the other `data.models` imports at the top:
...
```bash
git add apps/virtual_tryon_android/app/src/main/java/aivastra/nice/interactive/data/models/TryOnHistoryModels.kt \
        apps/virtual_tryon_android/app/src/main/java/aivastra/nice/interactive/api/ApiService.kt \
        apps/virtual_tryon_android/app/src/main/java/aivastra/nice/interactive/data/repository/TryOnHistoryRepository.kt
git commit -m "$(cat <<'EOF'
