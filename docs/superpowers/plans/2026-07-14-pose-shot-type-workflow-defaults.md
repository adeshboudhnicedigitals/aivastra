...
```bash
pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts test/integration/catalogue-template-subcategories-admin.test.ts
```
...
```bash
pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts test/integration/shot-type-workflow-resolve.test.ts
```
...
- [ ] `pnpm --filter @aivastra/api test` — full unit suite green
- [ ] `pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts` — full integration suite green (aside from the pre-existing unrelated `catalog.test.ts` failure documented in `vitest.config.ts`)
- [ ] `pnpm --filter @aivastra/api typecheck` and `pnpm --filter @aivastra/admin typecheck` — both clean
- [ ] `pnpm lint` — clean
