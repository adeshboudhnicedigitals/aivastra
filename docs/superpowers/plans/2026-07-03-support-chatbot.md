...
- Logger: `createLogger(service)` from `@aivastra/logger`; **no `console.log`**.
...
Run: `pnpm --filter @aivastra/db build && pnpm typecheck`
Expected: PASS.
...
### Task 2: Shared Zod types (`@aivastra/types`)
...
Run: `pnpm --filter @aivastra/types build && pnpm typecheck`
Expected: PASS.
...
export interface ChatbotDeps {
  env: Env;
  db: DB;               // from @aivastra/db createDb
...
```json
{
  ...
  "dependencies": {
    ...
    "@aivastra/observability": "workspace:*",
...
Run: `pnpm --filter @aivastra/chatbot test`
...
```ts
import { schema, sql, type DB } from '@aivastra/db';
...
Run: `pnpm --filter @aivastra/chatbot test -- ingest`
...
```ts
import { eq, schema, type DB } from '@aivastra/db';
import type { Logger } from '@aivastra/logger';
import type { Redis } from 'ioredis';
import { AppError } from '../lib/errors.js';
import type { EmbedFn } from '../server.js';
...
Run: `pnpm --filter @aivastra/chatbot test -- search`
...
```ts
import { sql, type DB } from '@aivastra/db';
import type { EmbedFn } from '../server.js';
...
**Interfaces:**
- Consumes: `requireAdmin` (`apps/api/src/modules/admin/guard.ts`), `QnaUpsert` from `@aivastra/types`, `schema.chatbotQna`.
...
Run: `pnpm --filter @aivastra/api test -- admin-chatbot`
Expected: FAIL — 404.
...
Run: `pnpm --filter @aivastra/admin dev` — log in as admin, create a Q&A pair, toggle active, hit Re-ingest (with `apps/chatbot` running via `pnpm --filter @aivastra/chatbot dev`), confirm status strip shows embedded count.
Run: `pnpm typecheck && pnpm --filter @aivastra/admin build`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/chatbot test -- conversation`
Expected: FAIL — module not found.
...
```ts
import { and, eq, inArray, schema, sql, type DB } from '@aivastra/db';
...
Run: `pnpm --filter @aivastra/chatbot test -- bot`
...
```ts
import { eq, schema, sql, type DB } from '@aivastra/db';
...
Run: `pnpm --filter @aivastra/chatbot test -- ws`
...
```ts
import { and, eq, schema, sql } from '@aivastra/db';
...
Run: `pnpm --filter @aivastra/chatbot test -- escalation`
Expected: FAIL — modules not found.
...
Run: `pnpm --filter @aivastra/api test -- admin-chatbot-hitl`
Expected: FAIL — 404.
...
```ts
import type { WsAgentFrameT, WsServerFrameT } from '@aivastra/types';
...
Add `"@aivastra/types": "workspace:*"` to `apps/admin-web/package.json` dependencies if not already present; `pnpm install`.
...
**Interfaces:**
- Consumes: chatbot `POST /ws-ticket` (user Bearer from `access_token` cookie), `/ws?ticket=`, `GET /conversations/:id/messages`; `WsClientFrameT`/`WsServerFrameT` from `@aivastra/types`; `C` tokens.
...
Run: `pnpm typecheck && pnpm --filter @aivastra/web build`
Expected: PASS.
...
Run: `pnpm --filter @aivastra/chatbot test -- ratelimit` → PASS.
...
- `CLAUDE.md`: add `apps/chatbot` to Monorepo Layout table + `pnpm --filter @aivastra/chatbot dev` command row.
- `docs/progress.md`: new dated entry — Done / Failed / Open Questions for this build.

- [ ] **Step 4: Full green check**

Run: `pnpm typecheck && pnpm lint && pnpm --filter @aivastra/chatbot test && pnpm --filter @aivastra/api test`
Expected: ALL PASS.
