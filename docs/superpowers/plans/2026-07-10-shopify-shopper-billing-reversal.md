...
**Goal:** Every try-on generation on a merchant's Shopify storefront is billed against the merchant's own aivastra credit balance (`shopifyStores.ownerUserId`'s credits) instead of requiring the shopper to log into their own aivastra account.
...
- Every task ends with `pnpm --filter @aivastra/api typecheck` clean and the relevant test file green before moving to the next task.
...
```bash
pnpm --filter @aivastra/api test -- customer-auth
```
...
```bash
pnpm --filter @aivastra/api test -- shopify-customer.test.ts
```
...
```
    data-app-base="{{ block.settings.app_base | default: 'https://app.aivastra.com' }}"
```
...
Try It On no longer asks shoppers to sign into an aivastra account —
