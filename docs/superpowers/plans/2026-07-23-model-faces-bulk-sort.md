...
**Interfaces:**
- Consumes: `schema.modelFaces` (from `@aivastra/db`), already imported at top of file.
...
With `pnpm docker:up` and the API running (`pnpm --filter @aivastra/api dev`), hit the endpoint:
