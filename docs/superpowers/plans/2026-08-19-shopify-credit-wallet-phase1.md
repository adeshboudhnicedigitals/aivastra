...
- **Unit tests:** `pnpm --filter @aivastra/api test:unit`
...
Run: `pnpm --filter @aivastra/api test:unit -- packs`
...
```bash
docker exec -i aivastra-postgres psql -U postgres -d aivastra -c "\d shopify_credit_purchases"
```
...
Remove `PAYG_PRICE_PER_TRYON_USD_CENTS` from the `@aivastra/types` import on line 11.
...
```bash
docker exec -i aivastra-postgres psql -U postgres -d aivastra -c "\d shopify_stores" | grep -E "plan_handle|billing_mode|payg_spend_cap|subscription_is_test"
```

Expected: **no output** — every column is gone.

```bash
docker exec -i aivastra-postgres psql -U postgres -d aivastra -c "\dt shopify_*"
```
...
- [ ] `pnpm lint` — clean
- [ ] `pnpm build` — clean
- [ ] `pnpm --filter @aivastra/api test:unit` — pass (the known `env.test.ts` local-`.env` failure aside)
