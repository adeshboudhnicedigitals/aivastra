...
- Produces:
  - `MerchantOnboardingBody`, `MerchantStatusSchema` in `@aivastra/types`
...
pnpm docker:up
pnpm db:migrate
pnpm --filter @aivastra/api test -- merchant-onboarding
```
Expected: PASS, 9 tests.
...
- Modify: `app/src/main/java/aivastra/nice/interactive/ApiUtils/APIConstant.kt:18-31`
- Modify: `app/src/main/java/aivastra/nice/interactive/viewmodel/category/SareeCategoryDataRepository.kt:35-121`
- Modify: `app/src/main/java/aivastra/nice/interactive/activity/auth/LoginActivity.kt`
- Create: `app/src/main/java/aivastra/nice/interactive/activity/auth/OnboardingActivity.kt`, `app/src/main/res/layout/activity_onboarding.xml`
...
1. In Google Cloud console, add an Android OAuth client with the debug keystore SHA-1 and package
   `aivastra.nice.interactive`. Put the existing **Web** client ID in `gradle.properties` as
   `GOOGLE_WEB_CLIENT_ID`.
2. `pnpm docker:up && pnpm --filter @aivastra/api dev`; point `API_BASE_URL` at that host.
3. `./gradlew :app:installDebug`. Sign in with a Google account never used with Aivastra → the
