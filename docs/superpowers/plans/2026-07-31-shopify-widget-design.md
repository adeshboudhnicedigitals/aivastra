...
3. **`WIDGET_COPY_DEFAULTS` lives in `apps/shopify/src/lib/widgetDefaults.ts`, not `packages/types`.** `apps/shopify/package.json` has no `@aivastra/types` dependency — it hand-duplicates types in `src/types.ts`, which keeps zod out of the SPA bundle. The server never needs the defaults: it stores nulls and Liquid supplies the fallbacks.
...
- Produces:
  - `ShopifyWidgetTheme`, `ShopifyWidgetCopy`, `ShopifyWidgetBehavior`, `ShopifyWidgetConfig` from `@aivastra/db`
  - `ShopifyStoreSettings.widget?: ShopifyWidgetConfig`
  - `ShopifyWidgetConfigPatch` (Zod schema + inferred type) from `@aivastra/types`
...
Run: `pnpm --filter @aivastra/api test -- metafields`
...
**Interfaces:**
- Consumes: `ShopifyWidgetConfigPatch` from `@aivastra/types`; `writeWidgetConfigMetafield(shop, accessToken, shopifyShopId, config, log, fetchFn?)` from `./metafields.js`; `getValidAccessToken(app, store)` from `./token.js`.
...
Run: `pnpm --filter @aivastra/api test -- shopify-widget-config`
...
> **Carry-over requirement:** if shopper-limits Task 6 has landed, `tryon-block.liquid` contains an email-gate step (`.aivastra-tryon__step--email` or similar). Copy that markup into the new file verbatim before deleting the old one. Verify with `git show HEAD:apps/shopify-extension/extensions/tryon-theme-extension/blocks/tryon-block.liquid | grep -i email` — if it prints nothing, Task 6 has not landed and there is nothing to carry.
...
Run: `pnpm --filter @aivastra/api test -- onboarding`
...
2. Delete these two attributes from the root `<div class="aivastra-tryon">`:
  ...
  "settings": [
    {
      ...
      "id": "api_base",
      "label": "API base URL",
      "default": "https://app.aivastra.com"
    },
    {
      "type": "text",
      "id": "promo_text",
...
**Interfaces:**
- Consumes: the metafield written by Task 2 — `shop.metafields.aivastra.widget_config.value` with shape `{ theme: { accentColor }, copy: {...}, behavior: {...} }`.
...
```liquid
{%- assign cfg = shop.metafields.aivastra.widget_config.value -%}
```
...
```liquid
    style="--aivastra-button-color: {{ block.settings.button_color }}; --aivastra-text-color: {{ block.settings.text_color }}; --aivastra-border-radius: {{ block.settings.border_radius }}px;{% if cfg.theme.accentColor %} --aivastra-accent: {{ cfg.theme.accentColor | escape }};{% endif %}"
```
...
| Current | Replacement |
|---|---|
| `<p class="aivastra-tryon__heading">Try It On</p>` | `<p class="aivastra-tryon__heading">{{ cfg.copy.heading \| default: 'Try It On' \| escape }}</p>` |
| `<p class="aivastra-tryon__subheading">See how it looks on you</p>` | `<p class="aivastra-tryon__subheading">{{ cfg.copy.subheading \| default: 'See how it looks on you' \| escape }}</p>` |
| `<h2 class="aivastra-tryon__upload-title">Ready to try it on?</h2>` | `<h2 class="aivastra-tryon__upload-title">{{ cfg.copy.uploadTitle \| default: 'Ready to try it on?' \| escape }}</h2>` |
...
```css
  background: var(--aivastra-accent, var(--aivastra-button-color, #000000));
```
...
- Produces: DOM classes `aivastra-tryon__result-actions`, `aivastra-tryon__add-to-cart`, `aivastra-tryon__share`, `aivastra-tryon__view-cart`, `aivastra-tryon__cart-error`, `aivastra-tryon__share-flash` — Task 7's drift test checks the preview only uses classes that exist here.
...
```css
.aivastra-tryon__result-actions {
  display: flex;
  align-items: stretch;
  gap: 10px;
  margin-top: 14px;
...
.aivastra-tryon__add-to-cart:disabled {
  opacity: 0.6;
  cursor: default;
}

.aivastra-tryon__share {
  flex: 0 0 auto;
  width: 48px;
...
.aivastra-tryon__cart-error {
  margin: 8px 0 0;
  font-size: 13px;
  color: #b42318;
}

.aivastra-tryon__share-flash {
...
Run: `pnpm --filter @aivastra/shopify-admin dev`
...
Run: `pnpm --filter @aivastra/shopify-admin typecheck && pnpm --filter @aivastra/shopify-admin build`
...
- Produces: `pnpm --filter @aivastra/shopify-admin test`.
...
Temporarily add `className="aivastra-tryon__nonexistent"` to a `<div>` in
...
Run: `pnpm --filter @aivastra/shopify-admin dev`, open `/widget-design`.
...
Run: `pnpm --filter @aivastra/shopify-admin typecheck && pnpm lint && pnpm --filter @aivastra/shopify-admin test`
Expected: clean.
...
pnpm --filter @aivastra/api test -- metafields
pnpm --filter @aivastra/api test -- onboarding
pnpm --filter @aivastra/api test -- shopify-widget-config
...
- [ ] `pnpm --filter @aivastra/shopify-admin build` succeeds — proves the cross-package CSS import resolves in Rollup, not only in the dev server.
