...
No test step: `apps/admin-web` has no test suite in this repo. Verified via typecheck/build and a manual check with `pnpm --filter @aivastra/admin dev`, same as the trial-credits UI task.
...
Run: `pnpm --filter @aivastra/admin dev`, open Settings, confirm the "Shopify Free Trial" section now also shows Starter/Growth/Pro credit fields loaded from `GET /admin/config` (1925/5000/22000 by default), edit one, click Save, reload, confirm it persists and the other two are unaffected.
