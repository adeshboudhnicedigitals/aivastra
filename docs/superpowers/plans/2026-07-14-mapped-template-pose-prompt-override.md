...
```bash
docker exec -it aivastra-postgres psql -U postgres -d aivastra -c "\d catalogue_template_pose_workflows"
```
...
```bash
pnpm --filter @aivastra/api test -- catalogue-template-subcategories-admin
```
