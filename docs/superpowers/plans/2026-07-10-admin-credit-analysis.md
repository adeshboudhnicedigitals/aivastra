...
- Schema changes go through `packages/db/src/schema/*.ts` + `pnpm --filter @aivastra/db run generate` — never hand-write migration SQL.
...
- After API tasks: `pnpm --filter @aivastra/api typecheck` must be clean. After admin-web tasks: `cd apps/admin-web && npx tsc -b --noEmit` must be clean (this package has no separate `typecheck` script).
...
No automated component tests exist for admin-web pages in this repo. Verify by hand in the dev server (`pnpm --filter @aivastra/admin dev`):
