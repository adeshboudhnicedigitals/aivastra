...
**Tech Stack:** Fastify 5, Drizzle ORM (Postgres), Redis (`config:system` JSON blob), Zod (`@aivastra/types`), Vitest integration tests (real Postgres/Redis/MinIO via `pnpm docker:up`), React (admin-web Vite SPA).
...
- All new/changed TypeScript must pass `pnpm --filter @aivastra/api typecheck` (or `pnpm typecheck` at the root) and `pnpm --filter @aivastra/api lint`.
...
Run: `pnpm --filter @aivastra/api test -- admin-config.test.ts -t "merchantFreeCredits"`
...
Run: `pnpm --filter @aivastra/admin build && pnpm --filter @aivastra/admin lint`
(the admin-web package has no standalone `typecheck` script — `build` runs `tsc -b` first)
Expected: no errors

- [ ] **Step 6: Manual verification**

Run: `pnpm --filter @aivastra/admin dev`, log in as SUPER_ADMIN, navigate to Settings → System, confirm "Merchant Free Credits" appears under Virtual Try-On Pricing, change the value, click Save, reload the page, confirm the value persisted.
...
Run: `pnpm --filter @aivastra/api test -- merchant-onboarding -t "grants the admin-configured free-credit"`
...
Run: `pnpm --filter @aivastra/api test -- merchant-onboarding.test.ts`
...
Run: `pnpm --filter @aivastra/api test -- merchant-onboarding merchant-api-keys device-merchant-status demo-catalog-merchant dev-api-auth`
...
Run: `pnpm --filter @aivastra/api test -- merchant-tryon.test.ts`
...
Run: `pnpm --filter @aivastra/api test -- kiosk-jobs.test.ts`
...
Run: `pnpm --filter @aivastra/api test -- merchant-me.test.ts`
