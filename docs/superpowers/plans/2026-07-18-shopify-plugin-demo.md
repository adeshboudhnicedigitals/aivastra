...
- Produces (used by Task 6 inside the iframe, and Task 8 in the parent page):
  - `EMBED_IMAGE_SELECTED: 'aivastra:image-selected'`
...
With `pnpm docker:up` and `pnpm --filter @aivastra/web dev` running, visit `/shopify-plugin`. Confirm:
...
With `pnpm docker:up` and `pnpm --filter @aivastra/web dev` running, log in as a merchant account and walk the entire flow from scratch:
