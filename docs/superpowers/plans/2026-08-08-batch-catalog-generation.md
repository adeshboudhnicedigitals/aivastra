...
**Interfaces:**
- Produces: the command `pnpm --filter @aivastra/api test:integration -- <path>`, used by every subsequent task.
...
Expected: `aivastra-postgres`, `aivastra-redis`, `aivastra-minio` all `Up ... (healthy)`.
...
```bash
pnpm --filter @aivastra/api test:integration -- test/integration/jobs-create-looks.test.ts
```
...
```bash
pnpm --filter @aivastra/api test:unit -- test/batch-row-rules.test.ts
```

Expected: FAIL — `"requiredInputsForPoses" is not exported by "@aivastra/types"`.
...
```bash
docker exec aivastra-postgres psql -U postgres -d tryon -c '\d jobs' | grep -E 'batch_id|jobs_batch_idx'
```
...
```bash
pnpm --filter @aivastra/api test:unit -- test/app-error-details.test.ts
```
...
**Interfaces:**
- Consumes: `resolveTryonPlan`, `verifyGarmentKey`, `TryonPlan` from `./create.js`; `atomicDeduct` from `../credits/ledger.js`; `promptGuard` from `./sanitize.js`; `AppError`, `withRowIndex` from `../../lib/errors.js`; `CreateBatchJobRequest`, `CreateBatchJobBody` from `@aivastra/types`.
...
```bash
pnpm --filter @aivastra/api test:integration -- test/integration/batch-jobs.test.ts
```
...
In `apps/api/src/modules/jobs/routes.ts`, add `CreateBatchJobRequest` to the `@aivastra/types` import, add `import { createBatchJobs } from './createBatch.js';` alongside the other create imports, and register directly after the `/v1/jobs/tryon` block:
...
**Interfaces:**
- Consumes: `DEFAULT_MAX_BATCH_JOBS` from `@aivastra/types`; `countBatchJobs` from `@aivastra/types`.
...
```typescript
import { countBatchJobs, type CreateBatchJobBody, JOB_SOURCE } from '@aivastra/types';
...
```bash
pnpm --filter @aivastra/api test:integration -- test/integration/batch-jobs.test.ts 2>&1 | grep -c 'select'
```
...
pnpm --filter @aivastra/api test:integration -- test/integration/batch-jobs.test.ts
pnpm --filter @aivastra/api test:integration -- test/integration/jobs-create-looks.test.ts
pnpm --filter @aivastra/api test:integration -- test/integration/jobs-create-mannequin.test.ts
pnpm --filter @aivastra/api test:integration -- test/integration/jobs-create-background-ownership.test.ts
```
...
**Interfaces:**
- Consumes: `requiredInputsForPoses`, `countBatchJobs` from `@aivastra/types`.
- Produces:
