...
**Tech Stack:** Fastify 5 + Zod (`@aivastra/types`), Drizzle ORM (`packages/db`), R2 storage (`@aivastra/storage`), React (admin-web Vite SPA), Vitest integration tests against real Postgres/Redis.
...
Run: `pnpm --filter @aivastra/db exec tsc --noEmit && pnpm --filter @aivastra/storage exec tsc --noEmit`
Expected: no output.
...
Add `AssetContentType` to the `@aivastra/types` import (currently line 2):
```ts
