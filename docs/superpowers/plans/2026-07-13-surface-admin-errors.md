...
**Tech Stack:** Markdown skill file with YAML frontmatter, ripgrep/grep enumeration, `pnpm --filter @aivastra/admin typecheck` for verification.
...
- Verification after fixes: `pnpm --filter @aivastra/admin typecheck` must pass.
...
Run: `pnpm --filter @aivastra/admin typecheck 2>&1 | tail -5`
Expected: the command resolves (the `@aivastra/admin` filter and a `typecheck` script exist) and completes. A clean tree should pass; record the result.
...
- Verification `pnpm --filter @aivastra/admin typecheck` → Task 1 Step 5; Task 2 Step 4. ✓
...
**Type consistency:** Helper names consistent throughout: `apiErrorMessage(err, fallback)`, `ApiError`, `toast({ kind, title, body })`, filter `@aivastra/admin`. ✓
