...
- Produces:
  - `SHOPIFY_CLIENT_EVENT_TYPES`, `SHOPIFY_REFUSAL_EVENT_TYPES`, `ShopifyWidgetEventRequest` from `@aivastra/types`
...
Run: `pnpm --filter @aivastra/api test -- shopify-events`
...
Run: `pnpm --filter @aivastra/api test -- shopify-refusal-events`
...
Run: `pnpm --filter @aivastra/api test -- shopify-limits`
...
Run: `pnpm --filter @aivastra/api test -- shopify-analytics`
...
- Produces:
  - `ShopifyAnalyticsQuery` from `@aivastra/types`
...
```ts
import type { schema as dbSchema } from '@aivastra/db';
import { ShopifyAnalyticsQuery } from '@aivastra/types';
import type { FastifyInstance } from 'fastify';
import {
  type AnalyticsRange,
  analyticsCards,
...
Run: `pnpm --filter @aivastra/dispatcher test -- shopify-events-retention`
...
| Where | Call |
|---|---|
| The click handler that unhides `.aivastra-tryon__modal` | `trackEvent('button_click');` |
...
Run: `pnpm --filter @aivastra/shopify-admin dev`, open `/analytics`.
...
Run: `pnpm --filter @aivastra/shopify-admin typecheck && pnpm lint`
Expected: clean.
...
pnpm --filter @aivastra/api test -- shopify-events
pnpm --filter @aivastra/api test -- shopify-refusal-events
pnpm --filter @aivastra/api test -- shopify-analytics
pnpm --filter @aivastra/dispatcher test -- shopify-events-retention
...
- [ ] `pnpm --filter @aivastra/shopify-admin build` succeeds.
