...
- Never use `console.log` in committed code — `@aivastra/logger` only (not touched by this plan; no new logging needed).
...
Run: `pnpm --filter @aivastra/api exec vitest run --config vitest.integration.config.ts test/integration/catalog-video-create.test.ts test/integration/catalogues-exclude-mannequin.test.ts`
...
Run: `pnpm --filter @aivastra/api exec tsc --noEmit -p .` and `pnpm --filter @aivastra/types exec tsc --noEmit -p .` (adjust filter name if different — check `packages/types/package.json`'s `"name"` field first).
...
Run: `pnpm docker:up` (if not already running), then `pnpm --filter @aivastra/web dev` and `pnpm --filter @aivastra/api dev` in separate terminals (or `pnpm dev` for everything via turbo).
