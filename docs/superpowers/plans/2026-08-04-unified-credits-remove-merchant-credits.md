...
**Interfaces:**
- Consumes: `schema.userCredits`, `schema.creditLedger` from `@aivastra/db`.
...
- Remove `MERCHANT_FREE_CREDITS,` from the `@aivastra/types` import block at lines 1–8.
...
pnpm --filter @aivastra/api typecheck
```
Expected: both clean. The types rebuild is mandatory — `apps/api` resolves `@aivastra/types` from `dist`, so skipping it produces a misleading `TS2305`.
...
**Interfaces:**
- Consumes: `schema.merchants`, `schema.userCredits`, `schema.creditLedger` from `@aivastra/db`.
...
```bash
docker exec -i aivastra-postgres psql -U postgres -d tryon -c "SELECT count(*) FROM credit_ledger WHERE reason = 'MERCHANT_CREDITS_MIGRATION';"
```
...
**Interfaces:**
- Produces: `schema.merchantCredits` and `schema.merchantCreditLedger` no longer exist on the `@aivastra/db` schema namespace.
