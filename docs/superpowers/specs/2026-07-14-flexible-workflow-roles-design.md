...
`WorkflowInputs.upperGarmentFile` becomes optional (`string | undefined`); `faceSideFile`/`backgroundFile`/`poseFile` stay required. `applyWorkflowPatch`'s face/bg `requireNode` calls (`patcher.ts:81,83`) become conditional on `tmpl.faceNodeId`/`tmpl.bgNodeId` being set — when set, the corresponding input file must also be present or the same throw applies (face/background are still always collected by the studio per the unchanged confirmed decision above, so this should also be unreachable in normal operation, but the invariant must hold structurally).
