...
A new visual mock ("AiVastra Shopify Admin Dashboard" — a DC-tool prototype export with fake in-memory state, no real logic) was dropped in at
`AiVastra Shopify Admin Dashboard/AiVastra Admin.dc.html`. It shows a branded pink-to-amber gradient look for `apps/shopify` (the embedded Shopify admin app), replacing the current default Polaris styling.
