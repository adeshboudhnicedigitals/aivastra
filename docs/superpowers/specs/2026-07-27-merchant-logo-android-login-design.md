...
The Android app (one codebase, used both as a mounted in-store kiosk and as a merchant staff member's own phone — there is no separate kiosk backend or "mode") currently hardcodes the Aivastra logo. Replace that with the logged-in merchant's own uploaded logo, falling back to the app's existing bundled Aivastra default when no merchant logo is configured.
