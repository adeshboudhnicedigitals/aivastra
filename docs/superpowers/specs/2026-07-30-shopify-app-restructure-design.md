...
`apps/shopify` (the embedded Shopify admin app, package `@aivastra/shopify-admin`) is being
...
`pnpm --filter @aivastra/shopify-admin typecheck` plus a manual smoke in a real embedded admin:
