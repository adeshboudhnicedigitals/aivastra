...
```
window.parent.postMessage({ type: 'aivastra:image-selected', imageUrl, jobId, poseLabel }, window.location.origin)
```
...
- **Media grid** is the only interactive surface on the page: local React state `images: { id, url, source: 'seed' | 'aivastra' }[]`, seeded with one placeholder product photo. Alongside Shopify's plain "+" add-media tile sits a new branded tile, **"Generate with Ai Vastra"** (gradient border, small logo).
...
On each completed result, `GenerationPanel` shows an additional **"Use this image"** button (via the new `onUseImage` prop) that posts the `aivastra:image-selected` message to the parent. The panel's normal "View full catalogue →" link is hidden here (`hideCatalogueLink`) since navigating the iframe away from the wizard would strand the merchant mid-demo.
