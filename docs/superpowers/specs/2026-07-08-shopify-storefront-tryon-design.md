...
Shopify Storefront (product page)
  ...
  |  - reads shop.metafields.aivastra.*  |
  |  - reads product.id                  |
  +-------------------+-------------------+
                      | fetch() (cross-origin)
                      v
...
**Block schema:** minimal — `name` and `target: "section"` only. No merchant-configurable settings; the widget key and product ID come from Liquid globals (`shop.metafields.aivastra.widget_key`, `product.id`), not merchant input. Setup is "add block in theme editor," nothing else.
