...
> store the Aivastra try-on plugin's "Try It On" button sits inside; it is
...
> existing VPS, e.g. `wp-demo.aivastra.com`, its own Docker Compose project +
...
demo store: a proper WooCommerce theme skinned with Aivastra's brand colors,
...
- Parent: `storefront` (installed via `wp theme install storefront`, not
  ...
  install would fetch).
- Child: `storefront-aivastra`, created at
  `wordpress-plugin/local-wp/themes/storefront-aivastra/` (version-controlled,
...
- Visual changes are CSS-only, reusing the widget's existing brand palette
  (`--aivastra-primary: #0f172a`, `--aivastra-accent: #6366f1`, from
...
`/var/www/html/wp-content/themes/storefront-aivastra`, mirroring how the
...
This also means the Aivastra plugin's category-mapping table (Settings →
Aivastra Try-On) will have real work to do — ~15 categories to map to try-on
...
6. Records the source filename in postmeta `_aivastra_import_source`.

**Idempotency**: before creating a product, the script checks whether a
product already has `_aivastra_import_source` matching that filename, and
...
1. `wp theme list` confirms `storefront-aivastra` is active.
...
4. Settings → Aivastra Try-On → category mapping shows all real categories
