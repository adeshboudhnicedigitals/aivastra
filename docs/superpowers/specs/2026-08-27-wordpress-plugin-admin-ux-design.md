...
> **Scope:** The plugin's admin-facing surface — Settings → Aivastra Try-On
...
- Sets `aivastra_connected` / `aivastra_error` / `aivastra_category_map_saved`
...
```
① Full API key
   From your aivastra account → API Keys. Verified once against your
...
Submits to the existing `aivastra_tryon_connect` action — no change to
...
- **Refresh balance** (`<details>`, no JS): reveals a single full-key field.
  Submits to a *new* `aivastra_tryon_refresh` action that re-verifies via
...
- **Update connection keys** (`<details>`, no JS): reveals the same two-step
  form as the not-connected state, still posting to `aivastra_tryon_connect`.
  This is the path for actually rotating the widget key or switching which
  aivastra account is connected.
- **Disconnect**: a new `aivastra_tryon_disconnect` action (its own nonce,
  ...
  *and* the category mapping. Category mapping is cleared too because a
  fresh connect could point at a different aivastra account with an
...
- `set_widget_key_and_snapshot(string $widgetKey, string $companyName, int $credits, string $creditsAsOf): void`
  ...
  there are no other callers besides `Aivastra_Connection_Service::connect()`,
...
- `refresh(string $fullKey): array{ok: bool, error?: string}` — new method,
  ...
  writes to `Aivastra_Connection_Settings` directly.

**`admin/class-settings-page.php`**:
- `handle_connect()` — unchanged.
- `handle_refresh()` — new. `check_admin_referer('aivastra_tryon_refresh')`,
  reads one field (`aivastra_full_key`), calls `$service->refresh()`,
  redirects with `aivastra_refreshed=1` or `aivastra_error=...`.
- `handle_disconnect()` — new. `check_admin_referer('aivastra_tryon_disconnect')`,
  no fields, calls `$settings->clear()`, redirects with
  `aivastra_disconnected=1`.
- `render()` — branches on `$settings->get_company_name() !== null` for
  §3a/§3b, and now reads `$_GET['aivastra_connected']` /
  `aivastra_refreshed` / `aivastra_disconnected` / `aivastra_category_map_saved`
  / `aivastra_error` to render one dismissible WP admin notice
...
- `enqueue_assets(string $hookSuffix)` — new, hooked to
  ...
  when `$hookSuffix === 'settings_page_aivastra-tryon'` (the hook suffix WP
  assigns to a page registered via `add_options_page` with menu slug
  `aivastra-tryon`), so the stylesheet never loads on unrelated admin
  screens.
...
- **New file**: `admin/assets/settings-page.css`. Provides: `.aivastra-status-card`
...
- **Version bump**: `AIVASTRA_TRYON_VERSION` → `0.4.0` (the version this
