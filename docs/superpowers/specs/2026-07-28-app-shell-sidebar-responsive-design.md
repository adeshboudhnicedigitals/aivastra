...
No backend changes; no test runner in `apps/catalogues-web` (established convention this session) — verification is `pnpm --filter @aivastra/web typecheck`/`lint`/`build`, plus manual checks:
