...
A Shopify try-on spends the merchant's own AiVastra credits (`atomicDeduct`,
...
- No billing/plan system. Shopify installs spend AiVastra credits; top-up
  happens at app.aivastra.com. Anything in earlier drafts about plan quotas,
  reset dates, or upgrade CTAs is out of scope permanently.
- No external login for shoppers. A shopper must never see an AiVastra login.
...
1. On load the widget reads `aivastra_client_id` from localStorage, else mints
