...
aivastra.com web-app credits. `shopify_stores.ownerUserId` links a store to a
`users` row (set via an in-app "link your aivastra.com account" flow), and
...
- Shopify-store credits (trial grant + plan-sync grant) live in a balance
  scoped to the store, never mixing with any aivastra.com user's
...
into "aivastra credit activity" / "Shopify credit activity" tabs — that
