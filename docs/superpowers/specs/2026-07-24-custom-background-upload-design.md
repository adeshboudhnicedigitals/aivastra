...
- `POST /v1/backgrounds/mine/presign` — body `{ contentType, contentLength }` (reuse `PresignUploadBody` shape from `@aivastra/types`). Generates `id = randomUUID()`, `r2Key = keys.userBackground(req.userId, id)`, presigns a PUT (10MB cap, 300s expiry — mirrors admin background presign). Binds `upload:owner:{r2Key} = req.userId` in Redis (24h TTL), same H2 pattern as `uploads/routes.ts`. Returns `{ uploadUrl, r2Key, id, expiresIn }`.
