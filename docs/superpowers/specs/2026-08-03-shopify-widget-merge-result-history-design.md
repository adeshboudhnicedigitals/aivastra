...
`localStorage['aivastra_tryon_history']` (shape: `{resultUrl, createdAt, productTitle,
...
The static `<img class="aivastra-tryon__result-image">` + single `result-actions` block in
`tryon-button.liquid` is removed; the `result` step becomes an empty scrollable container
(`aivastra-tryon__result-list`) populated entirely by JS, mirroring today's `history-list`
pattern.
...
(`.aivastra-tryon`), alongside the existing `data-product-id` etc.:
...
(`aivastra-tryon__step--result`, `aivastra-tryon__result-image`, `aivastra-tryon__result-actions`,
`aivastra-tryon__add-to-cart`, `aivastra-tryon__share`) still appear in the liquid file — the test
