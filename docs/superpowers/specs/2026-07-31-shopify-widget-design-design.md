...
`aivastra.widget_key` through the identical `POST /metafields.json` path. Same
...
<p class="aivastra-tryon__heading">{{ cfg.copy.heading | default: 'Try It On' | escape }}</p>
```
...
Modal-internal surfaces currently use `var(--aivastra-button-color, #000000)` —
...
`--aivastra-accent` comes from the metafield, `--aivastra-button-color` from the
block setting. Unset accent renders exactly as today. The storefront trigger
button (line 131) keeps using `--aivastra-button-color` alone — it is the
...
<div class="aivastra-tryon__result-actions">
  {%- unless cfg.behavior.addToCart == false -%}
    <button type="button" class="aivastra-tryon__add-to-cart"
  ...
  {%- unless cfg.behavior.share == false -%}
    <button type="button" class="aivastra-tryon__share"
...
`.aivastra-tryon__view-cart` unhides, and
...
It renders `.aivastra-tryon__modal-content` and inward — deliberately not the
`.aivastra-tryon__modal` wrapper, which is `position: fixed` and would escape the
page. Class names are all `aivastra-tryon__*`-prefixed, so injecting this
...
`style={{ '--aivastra-accent': accent }}` on the preview root, so the same CSS
...
1. Parse `tryon-button.liquid` for `aivastra-tryon__*` class names inside the
