...
```
https://app.aivastra.com/v1/...
```

(`admin.aivastra.com/v1/...` also reaches the same API — either works, but use
`app.aivastra.com` as the canonical base.) Confirm the staging host with the
...
means `POST https://app.aivastra.com/v1/auth/device-login`.
