...
`D:\aivastra\webtool`, tracking `origin/dev` (no matching remote branch yet — this is a new
...
7. **No admin-web-style unit test harness exists for `apps/catalogues-web` pages** — Task 4's
   verification step is a manual browser walk (`pnpm --filter @aivastra/web dev`), not an
...
- `pnpm --filter @aivastra/api test:integration` green — the new/modified tests in
...
- `pnpm --filter @aivastra/web typecheck` and a successful `pnpm --filter @aivastra/web dev`
