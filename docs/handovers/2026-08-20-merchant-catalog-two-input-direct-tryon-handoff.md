...
`D:\aivastra\webtool`, already tracking `origin/dev` with 5 prior commits on it
...
- Every `- [ ]` checkbox for Tasks 1-8 in
  ...
  `- [x]`.
- `pnpm --filter @aivastra/api test:integration` green — the new/modified tests from Tasks 3
...
- `pnpm --filter @aivastra/web typecheck` clean and Task 8's manual browser walk done for
