...
`D:\aivastra\webtool`, tracking `origin/feat/admin-identity-authz-audit`. **Implement here,
...
- `pnpm --filter @aivastra/api test:integration` green (new `role-permissions.test.ts`
  plus no regressions in existing suites).
- `pnpm typecheck` and `pnpm lint` clean repo-wide.
- `pnpm --filter @aivastra/admin build` succeeds (or the equivalent `tsc -b` — check
