...
**Branch:** `feat/android-kiosk-backend` — already checked out locally at `D:\aivastra\webtool`, tracking `origin/feat/android-kiosk-backend`. Implement here, do not create a new branch. Current HEAD: `73d253d1`.
...
**Modify:** `apps/admin-web/src/App.tsx` (register the route), `apps/admin-web/src/components/Sidebar.tsx` (nav entry). Verify the admin app builds (`pnpm --filter @aivastra/admin build` or check the exact package filter name in `apps/admin-web/package.json`).
...
- Run the full API suite (`pnpm --filter @aivastra/api test`) and confirm it's fully green — by the end of Task 5 it was 395/395 across 48 files; expect that number to grow with Tasks 6-8's new tests.
...
5. **Biome/lint via `pnpm --filter @aivastra/api lint <path>` from the repo root can mis-resolve the path** (doubles the `apps/api` segment). If you hit that, `cd apps/api && npx biome check <relative-path>` works reliably.
...
- `pnpm --filter @aivastra/api test`, `pnpm typecheck`, `pnpm lint` all clean.
