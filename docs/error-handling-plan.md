...
`infra/observability/dashboards/aivastra-overview.json` has no error panels beyond HTTP
...
3. `infra/observability/dashboards/aivastra-overview.json` — add panels:
...
- `pnpm --filter @aivastra/api test` — 323/323 baseline, must not regress
