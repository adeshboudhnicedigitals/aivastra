...
import { type CatalogueImageOption, type CatalogueResponse, JobThumbnail } from './JobThumbnail';
...
// not required to have been generated by AI Vastra at all. Exported so the
// Motion Studio main-screen upload panel (SourcePanel / CataloguePickerModal)
// can hand off an already-picked image and skip straight to template
// selection.
...
export function CatalogVideoWizard({
  ...
  onClose: () => void;
  onCreated: () => void;
  // Set when the Motion Studio main screen (SourcePanel) already collected an
  // uploaded image before this wizard ever opened — skips step 1 straight to
  // template selection. "Back" from step 2 still lands on step 1 with this
  // image pre-selected, so nothing is lost by skipping it forward.
  initialSource?: ImageSource;
}): React.ReactElement {
  const qc = useQueryClient();
  const [step, setStep] = useState<1 | 2 | 3>(initialSource ? 2 : 1);
  const [sourceTab, setSourceTab] = useState<'existing' | 'upload'>(
    initialSource?.kind === 'upload' ? 'upload' : 'existing',
  );
  const [source, setSource] = useState<ImageSource | null>(initialSource ?? null);
