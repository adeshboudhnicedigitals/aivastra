...
const WORKER_ID = 'test-worker-merchant-widget';
...
describe('merchant widget job — result uploaded as WebP q90', () => {
  let env: TestEnv;
  let redis: Redis;
  let pub: Redis;
  let comfy: ComfyMock;
  ...
  async function seedMerchantWidgetJob() {
    const [user] = await env.db
      .insert(schema.users)
      .values({ email: `merchant-owner-${Date.now()}@test.com`, displayName: 'Merchant Owner' })
      .returning();
    ...
    const [template] = await env.db
      ...
      .values({
        slug: `merchant-widget-tpl-${Date.now()}`,
        label: 'Merchant widget test template',
        jsonContent: {
    ...
    const garmentKey = `merchant-garments/${merchant.id}/garment.jpg`;
    ...
    for (const key of [customerPhotoKey, garmentKey]) {
      await env.s3.send(
  ...
  it('uploads the result as image/webp at widget-outputs/{jobId}/result.webp', async () => {
    const { jobId } = await seedMerchantWidgetJob();
    ...
    expect(output?.resultKey).toBe(`widget-outputs/${jobId}/result.webp`);

    const obj = await env.s3.send(
      new GetObjectCommand({
        Bucket: env.r2Bucket,
        Key: `widget-outputs/${jobId}/result.webp`,
      }),
    );
