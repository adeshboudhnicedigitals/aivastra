...
export async function catalogRoutes(app: FastifyInstance) {
  app.get('/v1/catalog/:type', {
    ...
    if (poseIds.length > 0) {
      const links = await app.db.select({ catalogItemId: schema.poseCatalogItems.catalogItemId })
      ...
      const items = await app.db.select().from(schema.catalogItems).where(and(...conditions));
    ...
    const allCats = await app.db.select().from(schema.catalogCategories)
    ...
    const categorized = genderFiltered.filter((i) => i.categoryId != null && catIds.has(i.categoryId));
    const uncategorized = genderFiltered.filter((i) => i.categoryId == null);

    const enrichedCat = categorized.map((i) => ({ ...i, thumbnailUrl: app.storage.publicUrl(i.thumbnailKey) }));
    const enrichedUncat = uncategorized.map((i) => ({ ...i, thumbnailUrl: app.storage.publicUrl(i.thumbnailKey) }));

    const tree = buildTree(cats, enrichedCat);
    if (enrichedUncat.length > 0) {
      (tree as any[]).push({ id: 0, slug: 'other', label: 'Other', children: [], items: enrichedUncat });
    }
    return { type, tree };
  });
}
