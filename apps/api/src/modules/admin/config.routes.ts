...
export async function adminConfigRoutes(app: FastifyInstance) {
  ...
  app.get('/admin/stats', { preHandler: requireAdmin(['SUPER_ADMIN', 'MODERATOR', 'SUPPORT']) }, async () => {
    ...
    const [
      queueN, queueP, workersRaw,
      jobsTodayRows, jobsYesterdayRows,
      activeUsersTodayRows, activeUsersYesterdayRows,
      creditsTodayRows, creditsYesterdayRows,
      failed24hRows,
      jobsPerDayRows,
      recentFailures,
      stuckJobs,
      ...
      app.db.select({ c: count() }).from(schema.jobs).where(and(gte(schema.jobs.createdAt, yesterday), lt(schema.jobs.createdAt, todayStart))),

      app.db.select({ c: countDistinct(schema.jobs.userId) }).from(schema.jobs).where(gte(schema.jobs.createdAt, todayStart)),
      app.db.select({ c: countDistinct(schema.jobs.userId) }).from(schema.jobs).where(and(gte(schema.jobs.createdAt, yesterday), lt(schema.jobs.createdAt, todayStart))),

      app.db.select({ c: sum(schema.jobs.creditsCharged) }).from(schema.jobs)
        .where(and(eq(schema.jobs.status, 'COMPLETED'), gte(schema.jobs.completedAt, todayStart))),
      app.db.select({ c: sum(schema.jobs.creditsCharged) }).from(schema.jobs)
        .where(and(eq(schema.jobs.status, 'COMPLETED'), gte(schema.jobs.completedAt, yesterday), lt(schema.jobs.completedAt as any, todayStart))),

      app.db.select({ c: count() }).from(schema.jobs)
    ...
    for (const [id, v] of Object.entries(workersRaw)) {
      try {
        ...
        workers.push({ id, status: info.status ?? 'UNKNOWN', healthy: !!healthKey, lastSeen: info.lastSeen });
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      jobsPerDayLabels.push(i === 0 ? 'Today' : d.toLocaleDateString('en', { month: 'short', day: 'numeric' }));
    }
