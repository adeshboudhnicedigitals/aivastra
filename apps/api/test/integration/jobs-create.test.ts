...
describe('jobs-create', () => {
  ...
  async function registerUser(email: string) {
    ...
    return { token: res.json().accessToken, userId: JSON.parse(atob(res.json().accessToken.split('.')[1])).sub };
  }

  async function seedCatalog(suffix = '') {
    const [type] = await app.db.insert(schema.catalogTypes).values({ slug: `models${suffix}`, label: 'Models' }).returning();
    const [cat] = await app.db.insert(schema.catalogCategories).values({ typeId: type.id, slug: `women${suffix}`, label: 'Women' }).returning();
  ...
  async function grantCredits(userId: string, amount: number) {
    await app.db.insert(schema.userCredits).values({ userId, balance: amount })
  ...
  it('returns 402 when balance is 0', async () => {
    ...
    const body = { inputs: { upperGarmentKey: 'inputs/x/garment.jpg',
      modelCatalogId: m, poseCatalogId: p, backgroundCatalogId: b, lowerCatalogId: l } };
    const res = await app.inject({ method: 'POST', url: '/v1/jobs/tryon',
      headers: { authorization: `Bearer ${token}` }, payload: body });
